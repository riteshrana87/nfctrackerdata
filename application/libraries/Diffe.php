...
{
	var $error = '';
	var $insertedStyle = 'font-weight: bold;color:#27AE60';
...
*/
	...
	Function FormatDiffAsHtml($before, $after, &$difference)
	{
		...
		for($d = 0; $d < $td; ++$d)
		{
			...
			{
				case '=':
					$html .= HtmlSpecialChars(substr($before, $diff['position'], $diff['length']));
					break;
				case '-':
					//$html .= '<del'.$deletedStyle.'>'.nl2br(HtmlSpecialChars(substr($before, $diff['position'], $diff['length']))).'</del>';
					break;
				case '+':
					$html .= '<ins'.$insertedStyle.'>'.HtmlSpecialChars(substr($after, $diff['position'], $diff['length'])).'</ins>';
					break;
				default:
