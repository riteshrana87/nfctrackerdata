...
// });
    ...
    public function getNewMessage($imap, $id, $withbody = true, $ypid) {
        return $this->formatNewMessage($imap, $id, $withbody, $ypid);
    }

    /**
     * Function Updated by Dhara Bhalala on 13th Sep 2018
     * To resolve issue of inline and external attach at a time 
     * @param $id
     * @param bool $withbody
     * @return array
     */
     protected function formatNewMessage($imap, $id, $withbody = true,$ypid) {
        $header = imap_headerinfo($imap, $id);

        $header_to_data = imap_rfc822_parse_headers(imap_fetchheader($imap,$id));
        // fetch unique uid
        ...
        $subject2 = $this->decodeMimeStr($header->subject);
        $email = array(
            'to' => isset($header_to_data->to) ? $this->arrayToAddress($header_to_data->to) : '',
            'from' => $this->toAddress($header_to_data->from[0]),
            'date' => $header->date,
            'subject' => $subject2,
            'uid' => $uid,
            ...
            'header' => $header,
            'to_email' => $this->arrayToAddressemails($header_to_data->to),
            'from_email' => $this->toAddressEmail($header_to_data->from[0]),
            'reply_to_address' => isset($header_to_data->reply_to) ? $this->arrayToAddress($header_to_data->reply_to) : '',
            'reply_to_email' => isset($header_to_data->reply_to) ? $this->arrayToAddressemails($header_to_data->reply_to) : '',
            'message_no' => $header->Msgno
        );
        if (isset($header_to_data->cc))
            $email['cc'] = $this->arrayToAddressemails($header_to_data->cc);
        if (isset($header_to_data->bcc))
            $email['bcc'] = $this->arrayToAddressemails($header_to_data->bcc);

        // get email body
        if ($withbody === true) {
            $body = $this->getNewMessageBody($imap,$uid);
                ...
                if(isset($structure->parts[$i]->parts) && !empty($structure->parts[$i]->parts) && count($structure->parts[$i]->parts)>0){
                    for($y = 0; $y < count($structure->parts[$i]->parts); $y++) 
                    {
                        ...
                        if($attachments[$n]['is_attachment']){
                            $attachments[$n]['attachment'] = imap_fetchbody($imap, $id, ($i+1).'.'.($y+1));                            
                            ...
                            if(isset($structure->parts[$i]->parts[$y]->disposition) && $structure->parts[$i]->parts[$y]->disposition == 'attachment'){
                                    $attachments[$n]['size'] = $structure->parts[$i]->parts[$y]->bytes;
                            }else{
                                $attachments[$n]['size'] = '0';
                            }    
                    ...
                    if($attachments[$n]['is_attachment']){
                        $attachments[$n]['attachment'] = imap_fetchbody($imap, $id, $i+1);                        
                        ...
                        if(isset($structure->parts[$i]->disposition) && $structure->parts[$i]->disposition == 'attachment'){
     ...
     * @return array
     */
     protected function formatMessage($id, $withbody = true,$ypid) {
        $header = imap_headerinfo($this->imap, $id);

        $header_to_data = imap_rfc822_parse_headers(imap_fetchheader($this->imap,$id));
        // fetch unique uid
    ...
    /**
     * Function created by Dhara Bhalala on 14 Sep 2018
     * to filter File name
     * **/
...
// no sent folder found? create one
    ...
    protected function toAddress($headerinfos) {
        ...
        /*to resolved to get multiple recipients email id into TO filed*/
...
// if HTML body is empty, try getting text body
     ...
     * @param $uid message id
     */
    protected function getNewMessageBody($imap,$uid) {
...
//        die;
     ...
     * @author  Raja K
     */
    public function listMessages($page = 1, $per_page = 25, $sort = false, $ypid) {
...
//        } else
            ...
            if (count($result) > 0) {
                for ($i = 0; $i < count($result); $i++) {
                    $email[] = $this->formatMessage($result[$i]->msgno, true, $ypid);
                }
            }
        }
