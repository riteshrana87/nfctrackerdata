...
class AIEmailTemplate extends CI_Controller {
    ...
    public function index($page = '') {
        ...
        if (!empty($allflag) && ($allflag == 'all' || $allflag == 'changesorting' || $allflag == 'changesearch')) {
            $this->session->unset_userdata('email_template_data');
        }

        $searchsort_session = $this->session->userdata('email_template_data');
        //Sorting
        ...
        $table = AAI_EMAIL_TEMPLATE . ' as ch';
        $where = array("ch.status !=" => "2");
        $wehere_not_in = array();
        $fields = array("l.login_id, CONCAT(`firstname`,' ', `lastname`) as name, l.firstname, l.lastname,ch.template_id,ch.subject,ch.body,ch.status,ar.receipt_type");
        $join_tables = array(LOGIN . ' as l' => 'ch.created_by=l.login_id',AAI_RECEIPT . ' as ar' => 'ar.receipt_type=ch.recipient_type');
        ...
        if (!empty($searchtext)) {
            ...
            $where_search = '(
                                (CONCAT(`firstname`, \' \', `lastname`) LIKE "%' . $searchtext . '%" 
                                    OR ch.subject LIKE "%' . $searchtext . '%" 
                                    OR ch.body LIKE "%' . $searchtext . '%" 
        ...
        $this->session->set_userdata('email_template_data', $sortsearchpage_data);
        setActiveSession('email_template_data');
    ...
    public function view($id) {
        if (is_numeric($id)) {
            ...
            $table = AAI_EMAIL_TEMPLATE . ' as ch';
            $match = "ch.template_id = '" . $id . "' AND ch.status !=2 ";
            $fields = array("ch.*");
    ...
    public function formValidation($id = null) {
        $this->form_validation->set_rules('subject', 'subject', 'trim|required|min_length[2]|max_length[100]|xss_clean');
    }
    ...
    public function add() {
        ...
        $match = "ch.status !=2";
        $fields = array("ch.*");
        $data['receipt_type'] = $this->common_model->get_records($table, $fields, '', '', $match);
...
//            'variable' => $this->input->post('variable'),
        ...
        // Insert query
        if ($this->common_model->insert(AAI_EMAIL_TEMPLATE, $data)) {
            //if (true) {
    ...
    public function updatedata($template_id) {
        $status = "";
...
//             'variable' => $this->input->post('variable'),
        ...
        if ($this->common_model->update(AAI_EMAIL_TEMPLATE, $data, $where)) {
    ...
    public function deletedata($id) {
        ...
        if (!empty($id)) {
            $updateData['status'] = 2;
            $table = AAI_EMAIL_TEMPLATE;
            $strWhere = "template_id =" . $id;
