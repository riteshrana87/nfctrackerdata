...
class AdminConcerns extends CI_Controller {
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['button_header'] = array('menu_module' => 'ypc');
    ...
    public function insertModule() {
        ...
        /* care_home_id added by Dhara Bhalala as per db changes */
        $fields = array();
        $fields = array(
            'ypc_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE, 'auto_increment' => TRUE),
            ...
            'is_archive' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0, 'COMMENT' => '0-not archive,1- user archive,2 -manager archive'),
            'modified_date' => array('type' => 'DATE', 'unsigned' => TRUE, 'NOT NULL' => TRUE),
            'created_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE),
            ...
            'is_concluded' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0)
        );
        ...
        $this->dbforge->add_key('ypc_id', TRUE);
        if (!$this->db->table_exists('yp_concerns')) {
            $this->dbforge->create_table('yp_concerns');
        }
        ...
        $success = $this->common_model->insert(YPC_FORM, $data_list);
        //Insert Record in Database
        if ($success) {
            $ypc_id = $this->db->insert_id();
            $msg = $this->lang->line('module_add_msg');
            $this->session->set_flashdata('msg', "<div class='alert alert-success text-center'>$msg</div>");
            redirect(ADMIN_SITE . '/' . $this->viewname . '/edit/' . $ypc_id);
        } else {
            // error
    ...
    public function edit($id) {
        $formInfo = $this->common_model->get_records(YPC_FORM, '', '', '', array('ypc_form_id' => $id));
        if (!empty($formInfo)) {
            $this->formValidation();
            if ($this->form_validation->run() == FALSE) {
                if (is_numeric($id)) {
    ...
    public function updateModule() {
        $id = $this->input->post('ypc_form_id');
        $this->load->dbforge();
        $form_json_data = strip_tags($this->input->post('form_json_data'));
        $jsonData = json_decode($form_json_data, TRUE);
        ...
        /* care_home_id added by Dhara Bhalala as per db changes */
        $fields_data = array('ypc_id', 'yp_id', 'created_by', 'created_date', 'is_archive', 'modified_date', 'created_by', 'modified_by', 'signoff', 'draft', 'is_delete', 'care_home_id', 'is_concluded');
        $result = array_merge($fields_data, $fields);

        $val_data = $this->db->list_fields('yp_concerns');
        ...
        if ($this->db->table_exists('yp_concerns')) {
            $this->dbforge->add_column('yp_concerns', $updated_fields);
        }

        $valaa_data = $this->db->list_fields('yp_concerns');
        $delete = array();
        ...
        foreach ($delete as $delete_val) {
            if ($this->db->field_exists($delete_val, 'yp_concerns')) {
                $this->dbforge->drop_column('yp_concerns', $delete_val);
            }
        }
        ...
        $where = array('ypc_form_id' => $id);
        if ($this->common_model->update(YPC_FORM, $data_list, $where)) {
