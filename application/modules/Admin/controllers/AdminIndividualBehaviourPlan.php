...
class AdminIndividualBehaviourPlan extends CI_Controller {
    ...
    public function insertModule() {
        ...
        $this->dbforge->add_key('ibp_id', TRUE);
        if (!$this->db->table_exists('individual_behaviour_plan')) {
            $this->dbforge->create_table('individual_behaviour_plan');
        }
        ...
        if ($this->common_model->insert(IBP_FORM, $data_list)) {
    ...
    public function edit($id) {
        $formInfo = $this->common_model->get_records(IBP_FORM, '', '', '', array('ibp_form_id' => $id));
        if (!empty($formInfo)) {
            $this->formValidation();
            if ($this->form_validation->run() == FALSE) {
                if (is_numeric($id)) {
    ...
    public function updateModule() {
        ...
        $id = $this->input->post('ibp_form_id');
        $this->load->dbforge();
        $form_json_data = strip_tags($this->input->post('form_json_data'));
        $jsonData = json_decode($form_json_data, TRUE);
        $jsonDataUpdated = array();
        ...
        $fields_data = array('ibp_id', 'yp_id', 'created_date', 'modified_date', 'created_by', 'modified_by', 'signoff', 'is_previous_version', 'edit_slug', 'care_home_id');

        $result = array_merge($fields_data, $fields);
        $val_data = $this->db->list_fields('individual_behaviour_plan');
        ...
        if ($this->db->table_exists('individual_behaviour_plan')) {
            $this->dbforge->add_column('individual_behaviour_plan', $updated_fields);
        }

        $valaa_data = $this->db->list_fields('individual_behaviour_plan');
        $delete = array();
        ...
        foreach ($delete as $delete_val) {
            if ($this->db->field_exists($delete_val, 'individual_behaviour_plan')) {
                $this->dbforge->drop_column('individual_behaviour_plan', $delete_val);
            }
        }
        ...
        $where = array('ibp_form_id' => $id);
        if ($this->common_model->update(IBP_FORM, $data_list, $where)) {
