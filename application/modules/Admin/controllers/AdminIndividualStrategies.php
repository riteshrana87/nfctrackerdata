...
class AdminIndividualStrategies extends CI_Controller {
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['button_header'] = array('menu_module' => 'is');
    ...
    public function insertModule() {
        ...
        $this->dbforge->add_key('individual_strategies_id', TRUE);

        if (!$this->db->table_exists('individual_strategies')) {
            $this->dbforge->create_table('individual_strategies');
        }
        ...
        if ($this->common_model->insert(IS_FORM, $data_list)) {
    ...
    public function edit($id) {
        $formInfo = $this->common_model->get_records(IS_FORM, '', '', '', array('is_form_id' => $id));
        if (!empty($formInfo)) {
            $this->formValidation();
            if ($this->form_validation->run() == FALSE) {
                if (is_numeric($id)) {
    ...
    public function updateModule() {
        ...
        $fields_data = array('individual_strategies_id', 'yp_id', 'created_date', 'modified_date', 'created_by', 'modified_by', 'is_previous_version', 'edit_slug', 'care_home_id');
        $result = array_merge($fields_data, $fields);

        $val_data = $this->db->list_fields('individual_strategies');
        ...
        if ($this->db->table_exists('individual_strategies')) {
            $this->dbforge->add_column('individual_strategies', $updated_fields);
        }

        $valaa_data = $this->db->list_fields('individual_strategies');
        $delete = array();
        ...
        foreach ($delete as $delete_val) {
            if ($this->db->field_exists($delete_val, 'individual_strategies')) {
                $this->dbforge->drop_column('individual_strategies', $delete_val);
            }
        }
        ...
        $where = array('is_form_id' => $id);
        if ($this->common_model->update(IS_FORM, $data_list, $where)) {
            // echo "if";exit;
