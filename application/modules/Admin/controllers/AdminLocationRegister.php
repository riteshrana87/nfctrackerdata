...
class AdminLocationRegister extends CI_Controller {
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['button_header'] = array('menu_module' => 'lr');
    ...
    public function insertModule() {
        ...
        $this->dbforge->add_key('location_register_id', TRUE);

        if (!$this->db->table_exists('location_register')) {
            $this->dbforge->create_table('location_register');
        }
        ...
        if ($this->common_model->insert(LR_FORM, $data_list)) {
    ...
    public function edit($id) {
        $formInfo = $this->common_model->get_records(LR_FORM, '', '', '', array('lr_form_id' => $id));
        if (!empty($formInfo)) {
            $this->formValidation();
            if ($this->form_validation->run() == FALSE) {
                if (is_numeric($id)) {
    ...
    public function updateModule() {
        ...
        $fields_data = array('location_register_id', 'yp_id', 'created_date', 'modified_date', 'created_by', 'modified_by', 'edit_slug', 'is_delete', 'care_home_id');

        $result = array_merge($fields_data, $fields);

        $val_data = $this->db->list_fields('location_register');
        ...
        if ($this->db->table_exists('location_register')) {
            $this->dbforge->add_column('location_register', $updated_fields);
        }

        $valaa_data = $this->db->list_fields('location_register');
        $delete = array();
        ...
        foreach ($delete as $delete_val) {
            if ($this->db->field_exists($delete_val, 'location_register')) {
                $this->dbforge->drop_column('location_register', $delete_val);
            }
        }
        ...
        $where = array('lr_form_id' => $id);
        if ($this->common_model->update(LR_FORM, $data_list, $where)) {
            // echo "if";exit;
