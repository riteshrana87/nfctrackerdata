...
class AdminPlacementPlan extends CI_Controller {
    ...
    public function insertModule() {
       ...
       // $fields = array();
        $fields = array(
            'placement_plan_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE, 'auto_increment' => TRUE), 'yp_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'created_date' => array('type' => 'DATE', 'unsigned' => TRUE), 'modified_date' => array('type' => 'DATE', 'unsigned' => TRUE), 'created_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'modified_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'signoff' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'is_previous_version' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'edit_slug' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'care_home_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'pre_placement_info' => array('type' => 'longtext', 'constraint' => 5, 'unsigned' => TRUE), 'pre_placement_family' => array('type' => 'longtext', 'constraint' => 5, 'unsigned' => TRUE), 'pre_placement_edu' => array('type' => 'longtext', 'constraint' => 5, 'unsigned' => TRUE), 'pre_placement_relation' => array('type' => 'longtext', 'constraint' => 5, 'unsigned' => TRUE), 'pre_placement_school' => array('type' => 'longtext', 'constraint' => 5, 'unsigned' => TRUE), 'pre_placement_contact' => array('type' => 'longtext', 'constraint' => 5, 'unsigned' => TRUE), 'pre_placement_therapy' => array('type' => 'longtext', 'constraint' => 5, 'unsigned' => TRUE), 'pre_placement_appointment' => array('type' => 'longtext', 'constraint' => 5, 'unsigned' => TRUE));
        ...
        $this->dbforge->add_key('placement_plan_id', TRUE);
        if (!$this->db->table_exists('placement_plan')) {
            $this->dbforge->create_table('placement_plan');
        }
        ...
        if ($this->common_model->insert(PP_FORM, $data_list)) {
    ...
    public function edit($id) {
        $formInfo = $this->common_model->get_records(PP_FORM, '', '', '', array('pp_form_id' => $id));
        if (!empty($formInfo)) {
            $this->formValidation();
            if ($this->form_validation->run() == FALSE) {
                if (is_numeric($id)) {
    ...
    public function updateModule() {
        ...
        $fields_data = array('placement_plan_id', 'yp_id', 'created_date', 'modified_date', 'created_by', 'modified_by', 'signoff', 'is_previous_version', 'edit_slug', 'care_home_id', 'pre_placement_info', 'pre_placement_family', 'pre_placement_edu', 'pre_placement_relation', 'pre_placement_school', 'pre_placement_contact', 'pre_placement_therapy', 'pre_placement_appointment');
        $result = array_merge($fields_data, $fields);
        $val_data = $this->db->list_fields('placement_plan');
        ...
        if ($this->db->table_exists('placement_plan')) {
            $this->dbforge->add_column('placement_plan', $updated_fields);
        }

        $valaa_data = $this->db->list_fields('placement_plan');
        $delete = array();
        ...
        foreach ($delete as $delete_val) {
            if ($this->db->field_exists($delete_val, 'placement_plan')) {
                $this->dbforge->drop_column('placement_plan', $delete_val);
            }
        }
        ...
        $where = array('pp_form_id' => $id);
        if ($this->common_model->update(PP_FORM, $data_list, $where)) {
