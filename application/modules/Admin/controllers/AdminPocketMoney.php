...
class AdminPocketMoney extends CI_Controller {
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['button_header'] = array('menu_module' => 'ypf');
    ...
    public function insertModule() {
        ...
        $fields = array();
        $fields = array(
            'pocket_money_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE, 'auto_increment' => TRUE), 'yp_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'created_date' => array('type' => 'DATETIME', 'unsigned' => TRUE, 'NULL' => TRUE), 'modified_date' => array('type' => 'DATETIME', 'unsigned' => TRUE, 'NULL' => TRUE), 'created_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'modified_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'edit_slug' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'balance' => array('type' => 'TEXT', 'NULL' => TRUE), 'is_deleted' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'money_type' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'care_home_id' => array('type' => 'INT', 'constraint' => 11, 'unsigned' => TRUE, 'NULL' => TRUE));
        ...
        /* added by Ritesh Rana for solving invalid datatype issue on 09/10/2018 */
        ...
        $this->dbforge->add_key('pocket_money_id', TRUE);

        if (!$this->db->table_exists('pocket_money')) {
            $this->dbforge->create_table('pocket_money');
        }
        ...
        if ($this->common_model->insert(YPM_FORM, $data_list)) {
    ...
    public function edit($id) {
        $formInfo = $this->common_model->get_records(YPM_FORM, '', '', '', array('ypm_form_id' => $id));
        if (!empty($formInfo)) {
            $this->formValidation();
            if ($this->form_validation->run() == FALSE) {
                if (is_numeric($id)) {
    ...
    public function updateModule() {
        $id = $this->input->post('ypm_form_id');
        $this->load->dbforge();
        $form_json_data = strip_tags($this->input->post('form_json_data'));
        ...
        $fields_data = array('pocket_money_id', 'yp_id', 'created_date', 'modified_date', 'created_by', 'modified_by', 'edit_slug', 'balance', 'is_deleted', 'money_type', 'care_home_id');

        $result = array_merge($fields_data, $fields);

        $val_data = $this->db->list_fields('pocket_money');
        ...
        if ($this->db->table_exists('pocket_money')) {
            $this->dbforge->add_column('pocket_money', $updated_fields);
        }

        $valaa_data = $this->db->list_fields('pocket_money');
        $delete = array();
        ...
        foreach ($delete as $delete_val) {
            if ($this->db->field_exists($delete_val, 'pocket_money')) {
                $this->dbforge->drop_column('pocket_money', $delete_val);
            }
        }
        ...
        $where = array('ypm_form_id' => $id);
        if ($this->common_model->update(YPM_FORM, $data_list, $where)) {
    ...
    public function addClothingMoney() {
        $this->formValidation();
        if ($this->form_validation->run() == FALSE) {
            $data['crnt_view'] = $this->viewname;
            ...
            $data['footerJs'][3] = base_url('uploads/formbuilder/assets/js/demo.js');
            $data['footerJs'][4] = base_url('uploads/custom/js/formbuilder/formbuilder.js');
            $data['button_header'] = array('menu_module' => 'ycm');
            $data['form_data'] = array('button_data' => 'active');
            $data['main_content'] = $this->type . '/' . $this->viewname . '/addClothingMoney';
            $data['form_action_path'] = $this->type . '/' . $this->viewname . '/addClothingMoney';
            $this->parser->parse($this->type . '/assets/template', $data);
        } else {
            $this->insertClothingMoney();
        }
    }
    ...
    public function insertClothingMoney() {
        $this->load->dbforge();
        $form_json_data = strip_tags($this->input->post('form_json_data'));
        $jsonData = json_decode($form_json_data, TRUE);
        $fields = array();
        $fields = array(
            'clothing_money_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE, 'auto_increment' => TRUE), 'yp_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'created_date' => array('type' => 'DATETIME', 'unsigned' => TRUE, 'NULL' => TRUE), 'modified_date' => array('type' => 'DATETIME', 'unsigned' => TRUE, 'NULL' => TRUE), 'created_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'modified_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'edit_slug' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'balance' => array('type' => 'TEXT', 'NULL' => TRUE), 'is_deleted' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'care_home_id' => array('type' => 'INT', 'constraint' => 11, 'unsigned' => TRUE, 'NULL' => TRUE));
        ...
        $this->dbforge->add_field($fields);
        $this->dbforge->add_key('clothing_money_id', TRUE);

        if (!$this->db->table_exists('clothing_money')) {
            $this->dbforge->create_table('clothing_money');
        }
        ...
        //Insert Record in Database
        if ($this->common_model->insert(YCM_FORM, $data_list)) {
            $pp_id = $this->db->insert_id();
            $msg = $this->lang->line('module_add_msg');
            $this->session->set_flashdata('msg', "<div class='alert alert-success text-center'>$msg</div>");
            redirect(ADMIN_SITE . '/' . $this->viewname . '/editClothingMoney/' . $pp_id);
        } else {
            // error
            $msg = $this->lang->line('error_msg');
            $this->session->set_flashdata('msg', "<div class='alert alert-danger text-center'>$msg</div>");
    ...
    public function editClothingMoney($id) {
        $formInfo = $this->common_model->get_records(YCM_FORM, '', '', '', array('ycm_form_id' => $id));
        if (!empty($formInfo)) {
            $this->formValidation();
            if ($this->form_validation->run() == FALSE) {
                if (is_numeric($id)) {
                    ...
                    $data['button_header'] = array('menu_module' => 'ycm');
                    $data['form_data'] = array('button_data' => 'active');
                    $data['form_action_path'] = $this->type . '/' . $this->viewname . '/editClothingMoney/' . $id;
                    $data['main_content'] = $this->type . '/' . $this->viewname . '/editClothingMoney';
                    $this->parser->parse($this->type . '/assets/template', $data);
                } else {
                    show_404();
                }
            } else {
                $this->updateClothingMoney();
            }
        } else {
            show_404();
        }
    ...
    public function updateClothingMoney() {
        $id = $this->input->post('ycm_form_id');
        $this->load->dbforge();
        $form_json_data = strip_tags($this->input->post('form_json_data'));
        ...
        $fields_data = array('clothing_money_id', 'yp_id', 'created_date', 'modified_date', 'created_by', 'modified_by', 'edit_slug', 'balance', 'is_deleted', 'care_home_id');

        $result = array_merge($fields_data, $fields);

        $val_data = $this->db->list_fields('clothing_money');
        $diff_fields_result = array_merge(array_diff($result, $val_data));
        $updated_fields = array();
        ...
        if ($this->db->table_exists('clothing_money')) {
            $this->dbforge->add_column('clothing_money', $updated_fields);
        }

        $valaa_data = $this->db->list_fields('clothing_money');
        $delete = array();
        foreach ($valaa_data as $val) {
            if (!in_array($val, $result)) {
                $delete[] = $val;
        ...
        foreach ($delete as $delete_val) {
            if ($this->db->field_exists($delete_val, 'clothing_money')) {
                $this->dbforge->drop_column('clothing_money', $delete_val);
            }
        }
        ...
        //Update Record in Database
        $where = array('ycm_form_id' => $id);
        if ($this->common_model->update(YCM_FORM, $data_list, $where)) {
            $msg = $this->lang->line('module_update_msg');
            $this->session->set_flashdata('msg', "<div class='alert alert-success text-center'>$msg</div>");
            redirect(ADMIN_SITE . '/' . $this->viewname . '/editClothingMoney/' . $id);
        } else {
            // error
            $msg = $this->lang->line('error_msg');
            $this->session->set_flashdata('msg', "<div class='alert alert-danger text-center'>$msg</div>");
