...
class AdminRMP extends CI_Controller {
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['button_header'] = array('menu_module' => 'rmp');
    ...
    public function insertModule() {
        ...
        $fields = array();
        $fields = array(
            'rmp_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE, 'auto_increment' => TRUE), 'yp_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'created_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'created_date' => array('type' => 'DATETIME', 'unsigned' => TRUE), 'is_archive' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0, 'COMMENT' => '0-not archive,1- user archive,2 -manager archive'), 'modified_date' => array('type' => 'DATETIME', 'unsigned' => TRUE), 'created_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'modified_by' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'signoff' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'draft' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'is_delete' => array('type' => 'TINYINT', 'constraint' => 1, 'NOT NULL' => TRUE, 'DEFAULT' => 0), 'care_home_id' => array('type' => 'INT', 'constraint' => 5, 'unsigned' => TRUE), 'modified_time' => array('type' => 'TIME', 'unsigned' => TRUE, 'null' => TRUE));
        ...
        $this->dbforge->add_key('rmp_id', TRUE);
        if (!$this->db->table_exists('rmp')) {
            $this->dbforge->create_table('rmp');
        }
        ...
        if ($this->common_model->insert(RMP_FORM, $data_list)) {
    ...
    public function edit($id) {
        $formInfo = $this->common_model->get_records(RMP_FORM, '', '', '', array('rmp_form_id' => $id));
        if (!empty($formInfo)) {
            $this->formValidation();
            if ($this->form_validation->run() == FALSE) {
                if (is_numeric($id)) {
    ...
    public function updateModule() {
        $id = $this->input->post('rmp_form_id');
        $this->load->dbforge();
        $form_json_data = strip_tags($this->input->post('form_json_data'));
        $jsonData = json_decode($form_json_data, TRUE);
        $jsonDataUpdated = array();
        ...
        $fields_data = array('rmp_id', 'yp_id', 'created_by', 'created_date', 'is_archive', 'modified_date', 'created_by', 'modified_by', 'signoff', 'draft', 'is_delete', 'care_home_id', 'modified_time');
        $result = array_merge($fields_data, $fields);

        $val_data = $this->db->list_fields('rmp');
        ...
        if ($this->db->table_exists('rmp')) {
            $this->dbforge->add_column('rmp', $updated_fields);
        }

        $valaa_data = $this->db->list_fields('rmp');
        $delete = array();
        ...
        foreach ($delete as $delete_val) {
            if ($this->db->field_exists($delete_val, 'rmp')) {
                $this->dbforge->drop_column('rmp', $delete_val);
            }
        }
        ...
        $where = array('rmp_form_id' => $id);
        if ($this->common_model->update(RMP_FORM, $data_list, $where)) {
