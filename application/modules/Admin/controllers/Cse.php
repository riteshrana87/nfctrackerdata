...
class Cse extends CI_Controller {
    ...
    public function index($page = '') {
        ...
        $data['footerJs'][0] = base_url('uploads/custom/js/cse/cse.js');
        ...
        $table = NFC_ADMIN_CSE . ' as c';
        $match = "is_delete = '0'";
        $fields = array("c.*");
        $data['cse_data'] = $this->common_model->get_records($table, $fields, '', '', $match, '', $config['per_page'], $uri_segment, $sortfield, $sortby);
        ...
        if ($this->input->post('result_type') == 'ajax') {
            ...
            $data['main_content'] = $this->type . '/' . $this->viewname . '/cse';
    ...
    public function add() {
        ...
        $data['footerJs'][0] = base_url('uploads/custom/js/cse/cse.js');
        //Pass data In View
        $data['main_content'] = $this->type . '/' . $this->viewname . '/add_cse';
        $this->parser->parse($this->type . '/assets/template', $data);
    }

    /*
      @Author : Ishani Dave
      @Desc   : save CSE questions
     */
    ...
    public function edit($id) {
        if (is_numeric($id)) {
            //Get Records From  nfc_admin_cse Table
            $tableCse = NFC_ADMIN_CSE;
            $strWhere = "is_delete = '0' and cse_que_id = " . $id;
            $arrFields = array("*");
            $data['editRecord'] = $this->common_model->get_records($tableCse, $arrFields, '', '', $strWhere);
            if (empty($data['editRecord'])) {
                $msg = $this->lang->line('common_no_record_found');
                $this->session->set_flashdata('msg', "<div class='alert alert-danger text-center'>$msg</div>");
                redirect(ADMIN_SITE . '/' . $this->viewname);
            }
            $tblSub = NFC_ADMIN_CSE_SUB;
