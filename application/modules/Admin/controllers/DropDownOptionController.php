...
class DropDownOptionController extends CI_Controller {
    ...
    public function index($dropdownId, $page = '') {

        $table = AAI_DROPDOWN . ' as ch';
        $match = "ch.dropdown_id = '" . $dropdownId . "' AND ch.status !=2 ";
        $data['dropdownRecord'] = $this->common_model->get_records($table, array("ch.*"), '', '', $match);
        ...
        if (!empty($allflag) && ($allflag == 'all' || $allflag == 'changesorting' || $allflag == 'changesearch')) {
            $this->session->unset_userdata('dropdown_option_data');
        }
        $data['dropdownId'] = $dropdownId;

        $searchsort_session = $this->session->userdata('dropdown_option_data');
        //Sorting
        ...
        $config['base_url'] = base_url() . $this->type . '/' . $this->viewname . '/index/' . $dropdownId;
        if (!empty($allflag) && ($allflag == 'all' || $allflag == 'changesorting' || $allflag == 'changesearch')) {
        ...
        $table = AAI_DROPDOWN_OPTION . ' as ch';
        $where = array("ch.status !=" => "2", 'ch.dropdown_id' => $dropdownId);
        $wehere_not_in = array();
        $fields = array("l.login_id, CONCAT(`firstname`,' ', `lastname`) as name, l.firstname, l.lastname,ch.option_id,ch.title,ch.status");
        ...
        if (!empty($searchtext)) {
            ...
            $where_search = '(
                            ...
                            AND ch.status != "2" AND ch.dropdown_id=' . $dropdownId . ')';
...
//            pr($this->db->last_query());pre($data['information']);
        ...
        $this->session->set_userdata('dropdown_option_data', $sortsearchpage_data);
        setActiveSession('dropdown_option_data');
    ...
    public function view($id) {
        if (is_numeric($id)) {
            ...
            $table = AAI_DROPDOWN_OPTION . ' as ch';
            $match = "ch.option_id = '" . $id . "' AND ch.status !=2 ";
            $fields = array("ch.*");
    ...
    public function add($dropdownId) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['crnt_view'] = ADMIN_SITE . '/' . $this->viewname;
            $data['form_action_path'] = ADMIN_SITE . '/' . $this->viewname . '/add/' . $dropdownId;
            $data['main_content'] = $this->viewname . '/addEdit';
            $data['screenType'] = 'add';
            $data['dropdownId'] = $dropdownId;
            $data['id'] = '';
    ...
    public function insertData($dropdownId) {
        $data = array(
        ...
        // Insert query
        if ($this->common_model->insert(AAI_DROPDOWN_OPTION, $data)) {
            //if (true) {
        ...
        redirect(ADMIN_SITE . '/' . $this->viewname . '/index/' . $dropdownId);
    }
    ...
    public function edit($dropdownId, $id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['crnt_view'] = $this->viewname;
            $data['dropdownId'] = $dropdownId;
            $data['form_action_path'] = ADMIN_SITE . '/' . $this->viewname . '/edit/' . $dropdownId . '/' . $id;
            $data['main_content'] = $this->viewname . '/addEdit';
    ...
    public function updatedata($dropdownId, $option_id) {
        $status = "";
        ...
        if ($this->common_model->update(AAI_DROPDOWN_OPTION, $data, $where)) {
            //$msg = $this->lang->line('user_update_msg');
            $msg = 'Dropdown option has been updated successfully.';
            $this->session->set_flashdata('msg', "<div class='alert alert-success text-center'>$msg</div>");
        } else {    // error
    ...
    public function deletedata($dropdownId, $id) {

        if (!empty($id)) {
            $updateData['status'] = 2;
            $table = AAI_DROPDOWN_OPTION;
            $strWhere = "option_id =" . $id;
