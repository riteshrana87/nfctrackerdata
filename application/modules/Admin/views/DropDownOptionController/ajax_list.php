...
<div class="table-responsive">
    <table class="table table-bordered table-striped dataTable" id="example1" customer="grid" aria-describedby="example1_info">
        <thead>
            ...
            <tr customer="row">
            <th <?php if (isset($sortfield) && $sortfield == 'title') {
                if ($sortby == 'asc') {
                ...
                echo "class = 'sorting'";
            } ?> tabindex="0" aria-controls="example1" rowspan="1" colspan="1" style="width: 10%;" onclick="apply_sorting('title', '<?php echo $sorttypepass; ?>')">Title</th>
    ...
    foreach ($information as $row) {
        ...
        ?>
                    ...
                    <tr id="customer_ids">
                        ...
                        <td class="">
         <a class="btn btn-xs btn-primary" href="<?php echo base_url($crnt_view.'/view/'.$row['option_id']);?>" title="View Record"><i class="fa fa fa-file-text-o"></i></a>

         <a class="btn btn-xs btn-primary" href="<?= $this->config->item('admin_base_url') . $this->viewname; ?>/edit/<?= $dropdownId.'/'.$row['option_id'] ?>" title="Edit Record"><i class="fa fa-pencil"></i></a>
    <button class="btn btn-xs btn-danger" title="Delete Record"  onclick="delete_request_option(<?php echo $dropdownId; ?>,<?php echo $row['option_id']; ?>);"> <i class="fa fa-times"></i> </button>
