...
<div class="table-responsive">
    <table class="table table-bordered table-striped dataTable" id="example1" customer="grid" aria-describedby="example1_info">
        ...
        <tbody>
            <tr>   
                <?php if (isset($sdq_data) && count($sdq_data) > 0) { ?>
                    <?php
                    foreach ($sdq_data as $data) {
                        if ($data['status'] == 1) {
                    ...
                    <tr>
                        ...
                        <?php //if (checkAdminPermission('Rolemaster', 'edit')) { ?>
                                <a class="btn btn-xs btn-primary" href="<?php echo base_url($crnt_view . '/edit/' . $data['sdq_que_id']); ?>" title="<?= lang('edit') ?>" ><i class="fa fa-pencil"></i></a>
                                <?php //} ?>
                                <?php //if ($data['role_id'] != $role_id) { ?> <?php //if (checkAdminPermission('Rolemaster', 'delete')) { ?><a href="javascript:;" onclick="deleteSdq(<?php echo $data['sdq_que_id']; ?>);" class="btn btn-xs btn-danger" title="<?= lang('delete') ?>" ><i class="fa fa-times"></i></a><?php //} ?> <?php //} ?>
