    ...
    <div class="panel-heading aai-page-header" role="tab" id="headingl9">
        ...
        <div class="panel-body form-horizontal">
                                        ...
                                        <?php if ((!empty($row['subtype']) && $row['subtype'] == 'time') || $row['type'] == 'date') { ?>
                                                ...
                                                <div class="<?= (!empty($row['subtype']) && $row['subtype'] == 'time') ? 'input-group input-append bootstrap-timepicker' : '' ?><?= (!empty($row['type']) && $row['type'] == 'date') ? (!empty($row['description']) && $row['description'] == 'dob') ? 'input-group input-append date dob' : 'input-group input-append date adddate' : '' ?>">
                                                    <input type="<?= (!empty($row['type']) && $row['type'] == 'number') ? 'number' : ((!empty($row['subtype']) && $row['subtype'] != 'time') ? $row['subtype'] : 'text') ?>"  
                                                           ...
                                                           placeholder="<?= !empty($row['placeholder']) ? $row['placeholder'] : '' ?>"
							value="<?php if($row['name']=='l9_date_of_incident'){ echo $l1date_of_incident;}elseif($row['name']!='l9_time_incident'){?><?=!empty($l9_form_data[0][$row['name']])?nl2br(html_entity_decode($l9_form_data[0][$row['name']])):(isset($row['value'])?$row['value']:'')?><?php } else { echo $l1time_of_incident;?><?php }?>" <?= ($row['type'] == 'date') ? 'readonly' : '' ?> data-parsley-errors-container="#errors-container<?= $row['name'] ?>" <?= (!empty($row['subtype']) && $row['subtype'] == 'time') ? 'readonly' : '' ?> />
			...
			<?php      }else if($row['type'] == 'radio-group') { ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-12">
                                         ...
                                         foreach ($row['values'] as $select) {
                                          ...
                                          ?>
                                          <option value="<?=!empty($select['value'])?$select['value']:''?>" <?=(!empty($l9_form_data[0][$row['name']]) && $l9_form_data[0][$row['name']] == $select['value'])?'selected="true"':!empty($select['selected'])?'selected="true"':''?> ><?=!empty($select['label'])?$select['label']:''?></option>
                                        <?php } } } //select loop ?>
							...
							<?php 
							if($row['name']=='l9_form_status'){?>
								
								 <div class="col-md-4 col-sm-6">
										<div class="form-group">
											<h2 class="aai-header-title col-md-12">Report Compiler <span class="astrick">*</span></h2>
												<div class="col-sm-12">
													<input type="text" disabled="true" class="form-control" value="<?= (isset($l9_report_compiler))? $l9_report_compiler:$loggedInUser['FIRSTNAME'].' '.$loggedInUser['LASTNAME'] ?>">                                
													<input type="hidden" name="l9_report_compiler" class="form-control" value="<?= (isset($l9_report_compiler))? $l9_report_compiler:$loggedInUser['FIRSTNAME'].' '.$loggedInUser['LASTNAME'] ?>">
												</div> 
										</div>
								</div>      	
							<?php }?>				
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <h2 class="aai-form-ttle col-md-12"><?= !empty($row['label']) ? $row['label'] : '' ?></h22>
                                </div>
