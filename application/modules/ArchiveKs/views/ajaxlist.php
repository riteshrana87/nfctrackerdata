                        ...
                        if(!empty($form_data))
                        {
                            ...
                            <?php }else if(!empty($row['displayonlist'])){ ?>
                                <td> 
                                    ...
                                    .'...<a data-href="'.base_url('KeySession/readmore/'.$data['ks_id'].'/'.$row['name']).'" data-refresh="true" data-toggle="ajaxModal" class="btn">read more</a>' : (($row['type'] == 'date' && !empty($data[$row['name']]) && $data[$row['name']] !='0000-00-00') ? configDateTime($data[$row['name']]) : nl2br(html_entity_decode($data[$row['name']])))) :(isset($row['value'])?$row['value']:'') ?>
                                </td>
                            <?php } }} }?>
                                ...
                                <td class="text-center">
                                    <?php if($past_care_id == 0){?>
                                        <a href="<?php echo base_url($crnt_view.'/view/'.$data['ks_id'].'/'.$data['yp_id']);?>" class="btn btn-link btn-blue"><i class="fa fa-file-text-o" aria-hidden="true"></i></a>
                                <?php if(checkPermission('ArchiveKs','hidden_archive')){ ?>
                                <a href="<?php echo base_url($crnt_view.'/undoArchive/'.$data['ks_id'].'/'.$data['yp_id']);?>" class="btn btn-link btn-blue"><i class="fa fa fa-mail-reply" aria-hidden="true"></i></a>
                                <?php } ?>
                                    <?php }else{ ?>
                                    <a href="<?php echo base_url($crnt_view.'/view/'.$data['ks_id'].'/'.$data['yp_id'].'/'.$care_home_id.'/'.$past_care_id);?>" class="btn btn-link btn-blue"><i class="fa fa-file-text-o" aria-hidden="true"></i></a>
                                    <?php } ?>
                                 </td>   
                            </tr>
                        <?php } ?>
