...
<div id="mailTable" class="main-design">
    <div class="table-responsive" >
        ...
        <input class="" type="hidden" name="uri_segment" id="uri_segment" value="<?=!empty($uri_segment)?$ypid.'/'.$box_types.'/'.$care_home_id.'/'.$past_care_id.'/'.$uri_segment:$ypid.'/'.$box_types.'/'.$care_home_id.'/'.$past_care_id.'/0'?>"> 
        ...
        <div class="table table-bordered ">
            ...
            <!--    <li class="bd-in-stats"><a href="javascript:;"></a></li> --> <!-- this is not required and unnecessary li, commented by Jignesh shrimali on 26th Oct 2018 21:00 -->
                ...
                if (isset($sortfield) && $sortfield == 'from_mail') {
                    if ($sortby == 'asc') {
                        echo "class = 'col-sm-2 col-md-5 sorting_desc width-col'";
                    } else {
                ...
                <li class="bd-in-attach attach_icon attach_list"><a href="javascript:;"><i class="bd-attach-ico"></i></a></li>
                ...
                <ul class="inbox-table-body">
                    <?php if (count($mail_data) > 0) { ?>
						...
						//die;
                            ...
                            <li class="mail-tr <?php echo ($mail['is_flagged'] == 1) ? ' bd-in-mark' : ''; ?> "  data-forward="<?php echo base_url('ArchiveMail/forwardEmail/' . $mail['uid']); ?>" data-reply="<?php echo base_url('ArchiveMail/replyEmail/' . $mail['uid']); ?>" data-replyall="<?php echo base_url('ArchiveMail/replyEmailAll/' . $mail['uid']); ?>">
                          
                                <div class="col-sm-2 col-md-4 bd-desc tabl-style">
                                    <a title="<?php echo $mail['mail_subject'];?>" onclick="markasUnread('<?php echo $mail['uid']; ?>')" href="<?php echo base_url('ArchiveMail/viewThread/' . $mail['uid'].'/'.$ypid.'/'.$care_home_id.'/'.$past_care_id); ?>">
                                ...
                                <a  onclick="markasUnread('<?php echo $mail['uid']; ?>')" href="<?php echo base_url('ArchiveMail/viewThread/' . $mail['uid'].'/'.$ypid.'/'.$care_home_id.'/'.$past_care_id); ?>" >
 ...
 -->
                                <div class="bd-in-attach tabl-style">
                                    <?php
                                    if ($mail['file_size'] > 0) {
                                        $url = base_url('ArchiveMail/saveAttachment/' . $mail['email_unq_id'].'/'.$ypid);
