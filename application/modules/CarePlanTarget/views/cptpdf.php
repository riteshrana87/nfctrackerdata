...
.img-sp .image-property{width: auto !important;}
...
<section class="invoice">
    ...
    <div class="row invoice-info">
        <div class="col-xs-12">
            <p class="lead" style="margin-bottom: 15px; font-size: 18px; text-transform: uppercase; font-weight: 900;">Care Plan Target</p>
        </div>
    ...
    <div class="col-xs-12 padd_zero">
        ...
        if (!empty($cpt_form_data)) {
            ...
            foreach ($cpt_form_data as $row) {
                ...
                if ($row['type'] == 'textarea' || $row['type'] == 'radio-group' || $row['type'] == 'date' || $row['type'] == 'select' || $row['type'] == 'number' || $row['type'] == 'text' || $row['type'] == 'checkbox-group') {
                   $i++;   
                   ?>
                   <div class="<?= ($row['type'] == 'header') ? 'col-xs-6' : 'col-xs-6 ' ?> dont-break paddi-z " >
        ...
        <div class="col-xs-12 padd_zero">
            <p class="">
                ...
                <?php echo '<' . $head . ' class="page-title font-sml">'; ?>
        ...
        <tbody>
            <?php if (!empty($comments)) {
                foreach ($comments as $comment) { ?>
                    <tr>
                        <td>
                            <?= !empty($comment['cpt_comments']) ? $comment['cpt_comments'] : '' ?>

                        </td>
                        <td class="img-sp">
                            <?php
                            $fileViewArray = array(
                                'fileArray' => (isset($comment['cpt_attacchment']) && !empty($comment['cpt_attacchment']))? $comment['cpt_attacchment'] : '',
                                'filePathMain' => $this->config->item('cpt_img_base_url') . $ypid,
                                'filePathThumb' => $this->config->item('cpt_img_base_url_small') . $ypid
                            );
                            echo getFileViewPrint($fileViewArray);
