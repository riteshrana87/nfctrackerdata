...
<script>
    ...
    var ks_id = '<?=!empty($edit_data[0]['cpt_id'])?$edit_data[0]['cpt_id']:''?>';

</script>
<!-- main content start-->
<div id="page-wrapper">
    <div class="main-page">
        <form action="<?= base_url('CarePlanTarget/insert') ?>" method="post" id="ksform" name="ksform" data-parsley-validate enctype="multipart/form-data">
        ...
        <h1 class="page-title">
            ...
            <div class="pull-right for-tab">
                <div class="btn-group capsBtn">
                    <button type="submit" onclick="needToConfirm = false;"  class="btn btn-default" name="submit_ksform" id="submit_ksform" value="submit" style="default"><?php if(!empty($edit_data)){
                        echo "Update";
                    }else{
                        echo "Create";
                    } ?></button>
                    
                    <a href="<?=base_url($this->viewname .'/index/'.$ypid)?>" class="btn btn-default" style="default"><i class="fa fa-mail-reply"></i> Back To CPT</a> 
                    <a href="<?=base_url('YoungPerson/index/'.$YP_details[0]['care_home'])?>" class="btn btn-default" name="submit_ppform" id="submit_ppform" value="submit" style="default"><i class="fa fa-mail-reply"></i> CARE HOME YP LIST</a>            
                    <a href="<?= base_url('YoungPerson/view/' . $ypid) ?>" class="btn btn-default" style="default"><i class="fa fa-mail-reply"></i> Back To YP Info</a>
                </div>
            </div>
        </h1>
        <h1 class="page-title">
                ...
                if (!empty($cpt_form_data)) {
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="<?= (!empty($row['subtype']) && $row['subtype'] == 'time') ? 'input-group input-append bootstrap-timepicker' : '' ?><?= (!empty($row['type']) && $row['type'] == 'date') ? (!empty($row['description']) && $row['description'] == 'dob') ? 'input-group input-append date dob' : 'input-group input-append date dob' : '' ?>">
                                                    <input type="<?= (!empty($row['type']) && $row['type'] == 'number') ? 'number' : ((!empty($row['subtype']) && $row['subtype'] != 'time') ? $row['subtype'] : 'text') ?>"  
                                                           class="<?= !empty($row['className']) ? $row['className'] : '' ?> <?= ($row['type'] == 'date') ? 'cpt_adddate_prescribed ' : '' ?> <?= (!empty($row['subtype']) && $row['subtype'] == 'time') ? 'addtime' : '' ?> addtime_data"  
                                                           <?= !empty($row['required']) ? 'required=true' : '' ?>
                    ...
                    ?>
                            ...
                            <div class="">
                                ...
                                <input type="hidden" name="cpt_id" id="cpt_id" value="<?=!empty($edit_data[0]['cpt_id'])?$edit_data[0]['cpt_id']:''?>">
								      <input type="hidden" id="form_secret" name="form_secret"  value="<?php echo createFormToken();?>">
                                <button type="submit" onclick="needToConfirm = false;"  class="btn btn-default updat_bn" name="submit_ksform" id="submit_ksform" value="submit" onclick="$('#saveAsDraft').val(0);" style="default"><?php if(!empty($edit_data)){
                        echo "Update";
                    }else{
                        echo "Create";
                    } ?></button>
                      <button type="submit" class="btn btn-default updat_bn" name="draft_wrform" id="draft_wrform" style="default" onclick="$('#saveAsDraft').val(1);">SAVE AS DRAFT</button>
                        <input type="hidden" id="form_secret" name="form_secret"  value="<?php echo createFormToken();?>">
