...
class ClothingMoney extends CI_Controller {
    ...
    public function index($id, $careHomeId = 0, $isArchive = 0) {
        ...
        if (is_numeric($id)) {
            ...
            if (!empty($allflag) && ($allflag == 'all' || $allflag == 'changesorting' || $allflag == 'changesearch')) {
                $this->session->unset_userdata('cm_data');
            }

            $searchsort_session = $this->session->userdata('cm_data');
            //Sorting
            ...
            if ($isArchive == 0) {
                //get total balance
                $match = array('yp_id' => $id);
                $yp_clothing_money = $this->common_model->get_records(YP_CLOTHING_MONEY, '', '', '', $match);
                if (!empty($yp_clothing_money)) {
                    $data['total_balance'] = $yp_clothing_money[0]['total_balance'];
                }
            } else {
                $match = array('yp_id' => $id);
                $where_date = "created_date BETWEEN  '" . $created_date . "' AND '" . $movedate . "'";
                $yp_clothing_money = $this->common_model->get_records(CLOTHING_MONEY, '', '', '', $match);
                $balance_archive = end($yp_clothing_money);
            ...
            //get ks form
            $match = array('ycm_form_id' => 1);
            $ks_forms = $this->common_model->get_records(YCM_FORM, '', '', '', $match);
            if (!empty($ks_forms)) {
                $data['form_data'] = json_decode($ks_forms[0]['form_json_data'], TRUE);
            }
            $this->session->set_userdata('cm_data', $sortsearchpage_data);
            setActiveSession('cm_data'); // set current Session active
            $data['header'] = array('menu_module' => 'YoungPerson');
            $data['crnt_view'] = $this->viewname;
            $data['footerJs'][0] = base_url('uploads/custom/js/ClothingMoney/ClothingMoney.js');
        ...
        if (is_numeric($id)) {
            ...
            if (!empty($ks_forms)) {
                ...
                foreach ($data['ks_form_data'] as $form_data) {
                    $formField[] = $form_data['name'];
                }
            }
            $data['form_field_data_name'] = $formField;
            //get YP information
            $match = array("yp_id" => $id);
            $fields = array("yp_id,care_home,yp_fname,yp_lname,date_of_birth");
            $data['YP_details'] = $this->common_model->get_records(YP_DETAILS, $fields, '', '', $match);
    ...
    public function insert() {
        ...
        unset($postData['submit_cmform']);
        //get pp form
        $match = array('ycm_form_id' => 1);
        $ks_forms = $this->common_model->get_records(YCM_FORM, array("*"), '', '', $match);
        if (!empty($ks_forms)) {
                ...
                if (isset($row['name'])) {
                    if ($row['type'] == 'file') {
                        ...
                        if (!empty($_FILES[$filename]['name'][0])) {
                            ...
                            createDirectory(array($this->config->item('cm_base_url'), $this->config->item('cm_base_big_url'), $this->config->item('cm_base_big_url') . '/' . $postData['yp_id']));

                            $file_view = $this->config->item('cm_img_url') . $postData['yp_id'];
                            //upload big image
                            ...
                            if (!empty($upload_data)) {
                                foreach ($upload_data as $imageFiles) {
                                    /* common function replaced by Dhara Bhalala on 29/09/2018 */
                                    createDirectory(array($this->config->item('cm_base_small_url'), $this->config->item('cm_base_small_url') . '/' . $postData['yp_id']));

                                    /* condition added by Dhara Bhalala on 21/09/2018 to solve GD lib error */
                                    if ($imageFiles['is_image'])
                                        $a = do_resize($this->config->item('cm_img_url') . $postData['yp_id'], $this->config->item('cm_img_url_small') . $postData['yp_id'], $imageFiles['file_name']);
        ...
        $yp_clothing_money = $this->common_model->get_records(YP_CLOTHING_MONEY, '', '', '', $match);
        if (!empty($yp_clothing_money)) {
            $bal = $yp_clothing_money[0]['total_balance'];
            ...
            $this->common_model->update(YP_CLOTHING_MONEY, $update_pocket_blance, array('yp_id' => $postData['yp_id']));
        } else {
            ...
            //update total balance
            $this->common_model->insert(YP_CLOTHING_MONEY, $update_pocket_blance);
        }

        if (!empty($postData['clothing_money_id'])) {
            $data['balance'] = $total_balance;
            $data['clothing_money_id'] = $postData['clothing_money_id'];
            ...
            $this->common_model->update(CLOTHING_MONEY, $data, array('clothing_money_id' => $postData['clothing_money_id']));
            //Insert log activity
            $activity = array(
                'user_id' => $this->session->userdata['LOGGED_IN']['ID'],
                'yp_id' => !empty($postData['yp_id']) ? $postData['yp_id'] : '',
                'module_name' => CLOTHING_MONEY_MODULE,
                'module_field_name' => '',
                'type' => 2
            );
            ...
            if (!empty($data)) {
                ...
                $data['care_home_id'] = $data_yp_detail['YP_details'][0]['care_home'];
                $this->common_model->insert(CLOTHING_MONEY, $data);
                $data['clothing_money_id'] = $this->db->insert_id();
                //Insert log activity
                $activity = array(
                    'user_id' => $this->session->userdata['LOGGED_IN']['ID'],
                    'yp_id' => !empty($postData['yp_id']) ? $postData['yp_id'] : '',
    ...
    public function readmore($id, $field) {
        $params['fields'] = [$field];
        $params['table'] = CLOTHING_MONEY;
        $params['match_and'] = 'clothing_money_id=' . $id . '';
    ...
    public function DownloadPrint($yp_id, $section = NULL) {
        ...
        $data['edit_data'] = $this->common_model->get_records($table, $fields, $join_tables, 'left', '', '', '', '', 'ks.clothing_money_id', 'desc', '', $where);
        $data['ypid'] = $yp_id;

        $data['main_content'] = '/cmpdf';
        $data['section'] = $section;
        $html = $this->parser->parse('layouts/PDFTemplate', $data);
        $pdfFileName = "ypf-cm" . $yp_id . ".pdf";
    ...
    public function generateExcelFile($ypid) {
        ...
        if (!empty($ypid)) {
            ...
            $recordData = $this->common_model->get_records($table, $fields, $joinTables, 'left', '', '', '', '', 'clothing_money_id', 'desc', '', $where);
            ...
            if (!empty($formsdata)) {
                ...
                $this->excel->setActiveSheetIndex(0)->setTitle('Clothing Money');
                $sheet->getStyle('A1:Z1')->getFont()->setBold(true);
                $sheet->setCellValue('A1', !empty($yp_clothing_money[0]['total_balance']) ? ' Clothing Money Balance : ' . $yp_clothing_money[0]['total_balance'] : ' Clothing Money Balance : ' . '0');
        ...
        $fileName = 'Clothing Money' . date('Y-m-d H:i:s') . '.xls'; // Generate file name
