    ...
    <div class="col-xs-12 m-t-10">
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
               ...
               <input class="" type="hidden" name="uri_segment" id="uri_segment" value="<?=!empty($uri_segment)?$ypid.'/'.$coms_id.'/'.$uri_segment:$ypid.'/'.$coms_id.'/0'?>">  
               <?php }else{ ?>
               <input class="" type="hidden" name="uri_segment" id="uri_segment" value="<?=!empty($uri_segment)?$ypid.'/'.$coms_id.'/'.$coms_id.'/'.$care_home_id.'/'.$past_care_id.'/'.$uri_segment:$ypid.'/'.$coms_id.'/'.$care_home_id.'/'.$past_care_id.'/0'?>">  
               <?php } ?>
                </tr>
                </thead>
                <tbody>
                <?php if (isset($information) && count($information) > 0) { ?>
                    <?php foreach ($information as $data) { ?>
                            ...
                            <tr>
                                ...
                                <td><?=!empty($data['created_date'])?configDateTimeFormat($data['created_date']):lang('NA'); ?></td>
                                <td><?=!empty($data['modified_date'])?configDateTimeFormat($data['modified_date']):lang('NA'); ?></td>
                                ...
                                <td class="text-center">
                                    ...
                                    /* condition added by Ritesh Ranan on 28/09/2018 to archive functionality */
                                     if($past_care_id == 0){ ?>
                                    <a href="<?php echo base_url($crnt_view.'/viewApprovalCom/'.$data['coms_signoff_details_id'].'/'.$ypid);?>" class="btn btn-link"><i class="fa fa-file-text-o" aria-hidden="true"></i></a>
                                    <?php 
                                    ...
                                    <a href="javascript:;" onclick="resendMail(<?php echo $data['coms_signoff_details_id'] ; ?>,<?php echo $data['yp_id'] ; ?>,<?php echo $data['coms_id'] ; ?>);" class="btn btn-default width_a" title="Resend">Resend</a>
                                    <?php } } ?>
                                    <?php }else{ ?>
                                    <a href="<?php echo base_url($crnt_view.'/viewApprovalCom/'.$data['coms_signoff_details_id'].'/'.$ypid.'/'.$care_home_id.'/'.$past_care_id);?>" class="btn btn-link"><i class="fa fa-file-text-o" aria-hidden="true"></i></a>
                                    <?php } ?>
