                       ...
                       if(!empty($form_data))
                        {
                            foreach ($form_data as $row) {
                                ...
                                <td> <?= (!empty($data[$row['name']]) && $data[$row['name']] !='0000-00-00') ? ((strlen (strip_tags($data[$row['name']])) > 50) ? substr (nl2br(strip_tags($data[$row['name']])), 0, 50)
                                    .'...<a data-href="'.base_url($crnt_view.'/readmore/'.$data['ypc_id'].'/'.$row['name']).'" data-refresh="true" data-toggle="ajaxModal" class="btn">read more</a>' : nl2br(html_entity_decode($data[$row['name']]))) :(isset($row['value'])?$row['value']:'') ?></td>
                                <?php } } }}?>
                                <td><?php echo $data['name']; ?></td>
                                <td><?php echo $data['care_home_name']; ?></td>
                                <td><?php 
				/* changes done by Dhara Bhalala for conclude option and get other details */
                                $ypc_title1 = $ypc_title2 = $ypc_title3 = '';
                                if($data['total_comments']>0){
                                    $ypc_title1 = ($data['total_comments'] == 1)? '<b>1</b> Comment' : '<b>'.$data['total_comments'].'</b> Comments';                                        
                                    $ypc_title2 = ' Last comment added by <b>' .$data['last_comment_added_by'].'</b>';
                                    $ypc_title3 = ' on <b>'.configDateTimeFormat($data['last_comment_added_on']).'</b>';
                                    echo $ypc_title1;
                                    echo '<br/>';
                                    echo $ypc_title2;
                                    echo '<br/>';
                            ...
                            <?php if(checkPermission('Concerns','view')){ ?>        
                                    <a href="<?php echo base_url($crnt_view.'/view/'.$data['ypc_id'].'/'.$data['yp_id']);?>" class="btn btn-link"><i class="fa fa-file-text-o" title="View" aria-hidden="true"></i></a>
                            <?php }
                               if($data['is_concluded'] == 0 && checkPermission('Concerns','Move_KS_YPC')){ ?>        
                                    <a href="<?php echo base_url($crnt_view.'/moveToKS/'.$data['ypc_id'].'/'.$data['yp_id']);?>" class="btn btn-default">Move To KS</a>
                               <?php }
				/* changes done by Dhara Bhalala for conclude option and get other details */
                               if($data['is_concluded'] == 0){$link1 = 'display:""';$link2 = 'display:none';}else{$link1 = 'display:none';$link2 = 'display:""';}
                               ?>
<!--                                    <a style="<?= $link1 ?>" id="conclude_option_<?= $data['ypc_id'] ?>" title="Conclude" onclick="conclude_ypc('<?php echo $data['ypc_id'] ?>','<?php echo $data['yp_id'] ?>');" href="javascript:;" class="btn btn-link"><i class="fa fa-check-square-o" aria-hidden="true"></i></a>-->
                                    <a style="<?= $link2 ?>" id="concluded_option_<?= $data['ypc_id'] ?>" title="Concluded" href="javascript:;" class="btn btn-link"><i style="color:#70a51a" class="fa fa-check-circle" aria-hidden="true"></i></a>
                            <?php   } ?>  
                            <?php if(!empty($data['draft']) && $data['draft'] == '1'){ ?>
                            <?php if(checkPermission('Concerns','delete')){ ?>
                                <a href="<?php echo base_url($crnt_view.'/edit_draft/'.$data['ypc_id'].'/'.$data['yp_id']);?>" title="Draft" class="btn btn-link"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                               <?php }?>  
                                <?php if(checkPermission('Concerns','delete')){ ?>
                                    <a onclick="delete_ypc('<?php echo $data['ypc_id'] ?>','<?php echo $data['yp_id'] ?>');" href="javascript:;"class="btn btn-link"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                <?php } ?>
                                    <?php } 
                                    }else{
                                        if(checkPermission('Concerns','view')){ ?>        
                                    <a href="<?php echo base_url($crnt_view.'/view/'.$data['ypc_id'].'/'.$data['yp_id'].'/'.$care_home_id.'/'.$isArchiveCareHomePage);?>" class="btn btn-link"><i class="fa fa-file-text-o" title="View" aria-hidden="true"></i></a>
                                 <?php } ?>
                                 <?php if(!empty($data['draft']) && $data['draft'] == '1'){ ?>
