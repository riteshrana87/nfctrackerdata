    ...
    <div class="col-sm-12 m-t-10">    
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                ...
                <input class="" type="hidden" name="uri_segment" id="uri_segment" value="<?= !empty($uri_segment) ?  $ypc_id . '/' . $ypid.'/'. $uri_segment :  $ypc_id . '/' . $ypid.'/0' ?>">  
                <?php }else{ ?>
                <input class="" type="hidden" name="uri_segment" id="uri_segment" value="<?=!empty($uri_segment)?$ypc_id.'/'.$ypid.'/'.$care_home_id.'/'.$isArchiveCareHomePage.'/'.$uri_segment:$ypc_id.'/'.$ypid.'/'.$care_home_id.'/'.$isArchiveCareHomePage.'/0'?>">  
                <?php }?>
                </tr>
                </thead>
                <tbody>
<?php if (isset($ypc_signoff_data) && count($ypc_signoff_data) > 0) { ?>
                            <?php foreach ($ypc_signoff_data as $data) { ?>
                            ...
                            <tr>
                                ...
                                <td class="text-center">   
                                    <?php /* condition added by Dhara Bhalala for carehome YP archive */
                                    if($isArchiveCareHomePage == 0){ ?>
                                    <a href="<?php echo base_url($crnt_view . '/reviewedConcern/' . $data['ypc_id'] . '/' . $data['yp_id'].'/'.$data['ypc_signoff_details_id']); ?>" class="btn btn-link btn-blue"><i class="fa fa-file-text-o" title="View" aria-hidden="true"></i></a>
                                    <?php 
                                    ...
                                    <a href="javascript:;" onclick="resendMail(<?php echo $data['ypc_signoff_details_id'] ; ?>,<?php echo $data['yp_id'] ; ?>,<?php echo $ypc_id; ?>);" class="btn btn-default width_a" title="Resend">Resend</a>
                                    <?php } } ?>
                                   <?php }else{ ?>
                                        <a href="<?php echo base_url($crnt_view . '/reviewedConcern/' . $data['ypc_id'] . '/' . $data['yp_id'].'/'.$data['ypc_signoff_details_id'].'/'.$care_home_id.'/'.$isArchiveCareHomePage); ?>" class="btn btn-link btn-blue"><i class="fa fa-file-text-o" title="View" aria-hidden="true"></i></a>                                    
                                   <?php } ?>
