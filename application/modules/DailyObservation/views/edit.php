...
<!-- main content start-->
<div id="page-wrapper">
<form action="<?= base_url('DailyObservation/update') ?>" method="post" id="ppform" name="ppform" data-parsley-validate enctype="multipart/form-data">
    <div class="main-page">
        ...
        <h1 class="page-title">
            ...
            <a href="<?= base_url('YoungPerson/view/' .$pass_yp_id) ?>" class=" btn btn-default" name="back_to_yp_info" id="back_to_yp_info" value="submit" style="default">Back To YP Info</a>
            </div>
        </h1>
        <h1 class="page-title">
            ...
            <small>Created Date:</small>  <?= (!empty($edit_data[0]['daily_observation_date']) && $edit_data[0]['daily_observation_date'] != '0000-00-00') ? configDateTime($edit_data[0]['daily_observation_date']) : '' ?>
        </h1>
        </div>
        <div class="row">
                            ...
                            <div class="col-sm-12">
                                <div class="panel panel-default tile tile-profile">
                                    <div class="panel-body">
                                        <h2><?= !empty($row['label']) ? $row['label'] : '' ?> <?= !empty($row['required']) ? '<span class="astrick">*</span>' : '' ?>
                                            <?php if(!checkPermission('DailyObservation', 'external_edit')) { ?>
                                            <small class="value"><?= !empty($food_data[0][$row['name']]) ? html_entity_decode($food_data[0][$row['name']]) : (isset($row['value']) ? $row['value'] : '') ?>
                                            </small> 
                                               <?php } ?> 
                                        </h2>

                                        <textarea class="<?= !empty($row['className']) ? $row['className'] : '' ?> <?= ($row['subtype'] == 'tinymce') ? 'tinyeditor' : '' ?>" <?= !empty($row['required']) ? 'required=true' : '' ?> name="<?= !empty($row['name']) ? $row['name'] : '' ?>" placeholder="<?= !empty($row['placeholder']) ? $row['placeholder'] : '' ?>" <?php if ($row['subtype'] != 'tinymce') { ?> <?= !empty($row['maxlength']) ? 'maxlength="' . $row['maxlength'] . '"' : '' ?> <?= !empty($row['rows']) ? 'rows="' . $row['rows'] . '"' : '' ?>
                                            <?php } ?> id="<?= !empty($row['name']) ? $row['name'] : '' ?>" ><?php if(checkPermission('DailyObservation', 'external_edit') ) { ?><?= !empty($food_data[0][$row['name']]) ? html_entity_decode($food_data[0][$row['name']]) : (isset($row['value']) ? $row['value'] : '') ?><?php } ?></textarea>
                                    </div>
                                </div>
                            </div>
                            <?php
                                ...
                                <div class="panel panel-default tile tile-profile">
                                    <div class="panel-body">
                                        <h2><?= !empty($row['label']) ? $row['label'] : '' ?> <?= !empty($row['required']) ? '<span class="astrick">*</span>' : '' ?> 
                                           ...
                                           <?php $st=!empty($food_data[0][$row['name']])?nl2br(htmlentities($food_data[0][$row['name']])):(isset($row['value'])?$row['value']:'');?>
                                           <?php  } ?>
                                            <?php if(!checkPermission('DailyObservation', 'external_edit')) { ?>
                                            <small class="value">
                                            <?=!empty($st)?$st:''?>
                                                ...
                                                <div class="<?= (!empty($row['subtype']) && $row['subtype'] == 'time') ? 'input-group input-append bootstrap-timepicker' : '' ?><?= (!empty($row['type']) && $row['type'] == 'date') ? (!empty($row['description']) && $row['description'] == 'dob') ? 'input-group input-append date dob' : 'input-group input-append date adddate' : '' ?>">
                                                    <input type="<?= (!empty($row['type']) && $row['type'] == 'number') ? 'number' : ((!empty($row['subtype']) && $row['subtype'] != 'time') ? $row['subtype'] : 'text') ?>"
                                                           ...
                                                           <?php if($row['subtype'] == 'time'){ ?>
                                                                value="<?=!empty($st)?timeformat($st):''?>" 
                                                            <?php }elseif($row['type'] == 'date'){ ?>
                                                                value="<?=!empty($st)?configDateTime($st):''?>"     
                                                            <?php }else{ ?>
                                                                value="<?=!empty($st)?$st:''?>" 
                                                           <?php } ?>
                                                           <?php } ?>
                        ...
                        ?>
                                                        <div class="<?= !empty($row['inline']) ? 'radio-inline' : 'radio' ?>">
                                                            <label ><input name="<?= !empty($row['name']) ? $row['name'] : '' ?>" <?= !empty($row['required']) ? 'required=true' : '' ?> 
                                                                           ...
                                                                           value="<?= !empty($radio['value']) ? $radio['value'] : '' ?>" <?= (!empty($food_data[0][$row['name']]) && $food_data[0][$row['name']] == $radio['value']) ? 'checked="checked"' : isset($radio['selected']) ? 'checked="checked"' : '' ?>  type="radio">
                                        ...
                                        <div class="checkbox-group">
                                            ...
                                            if (count($row['values']) > 0) {
                                                ...
                                                if (!empty($food_data[0][$row['name']])) {
                                                    $checkedValues = explode(',', $food_data[0][$row['name']]);
                                                }
                                            ...
                                            if (count($row['values']) > 0) {
                                                foreach ($row['values'] as $select) {
                                                    if (!empty($select['label'])) {
                                                        ?>
                                                        <option value="<?= isset($select['value']) ? $select['value'] : '' ?>" <?= (isset($food_data[0][$row['name']]) && $food_data[0][$row['name']] == $select['value']) ? 'selected="true"' : '' ?> ><?= !empty($select['label']) ? $select['label'] : '' ?></option>
                                                    <?php
                                                    }
                                                }
                                            } //select loop  
                            ...
                            <div class="col-sm-12">
                                <div class="panel panel-default tile tile-profile">
                                    <div class="panel-body">
                                        <h2><?= !empty($row['label']) ? $row['label'] : '' ?> <?= !empty($row['required']) ? '<span class="astrick">*</span>' : '' ?>
                                            <?php  $st = '';
                                                if(!empty($edit_data[0][$row['name']]) && is_json($edit_data[0][$row['name']])){ 
                                                $jdata = array();
                                                $jdata =json_decode($edit_data[0][$row['name']]);
                                                if(!empty($jdata))
                                                {
                                           ...
                                           <?php $st=!empty($edit_data[0][$row['name']])?nl2br(htmlentities($edit_data[0][$row['name']])):(isset($row['value'])?$row['value']:'');?>
                                           <?php  } ?>
                                            <?php if(!checkPermission('DailyObservation', 'external_edit')) { ?>
                                            <small class="value">
                                            <?=!empty($st)?$st:''?>
                                        ...
                                        <textarea class="<?= !empty($row['className']) ? $row['className'] : '' ?> <?= ($row['subtype'] == 'tinymce') ? 'tinyeditor' : '' ?>" <?= !empty($row['required']) ? 'required=true' : '' ?> name="<?= !empty($row['name']) ? $row['name'] : '' ?>" placeholder="<?= !empty($row['placeholder']) ? $row['placeholder'] : '' ?>" <?php if ($row['subtype'] != 'tinymce') { ?> <?= !empty($row['maxlength']) ? 'maxlength="' . $row['maxlength'] . '"' : '' ?> <?= !empty($row['rows']) ? 'rows="' . $row['rows'] . '"' : '' ?>
                                            <?php } ?> id="<?= !empty($row['name']) ? $row['name'] : '' ?>" ><?php if(checkPermission('DailyObservation', 'external_edit') ) { ?><?=!empty($st)?$st:''?><?php } ?></textarea>
                                    </div>
                                </div>
                            </div>
            <?php
                                                    ...
                                                    <?= !empty($row['step']) ? 'step="' . $row['step'] . '"' : '' ?>
                                                           ...
                                                           <?php }else{?>
                                                            ...
                                                            <?php }else{ ?>
                                                                value="<?=!empty($st)?$st:''?>" 
                                                           <?php } ?>
                                                           <?php } ?> <?= ($row['type'] == 'date') ? 'readonly' : '' ?> data-parsley-errors-container="#errors-container<?= $row['name'] ?>" />
                                                    <?php if (!empty($row['subtype']) && $row['subtype'] == 'time') { ?>
                                                        <span class="input-group-addon add-on"><i class="fa fa-clock-o"></i></span>
            <?php } ?>
                                                <?php if (!empty($row['type']) && $row['type'] == 'date') { ?>
        ...
        <?php
            ...
            ?>  
                     ...
                     <div class="col-sm-12 capsBtn">
                                <div class="">
                                <input type="hidden" name="do_id" id="do_id" value="<?= !empty($edit_data[0]['do_id']) ? $edit_data[0]['do_id'] : '' ?>">
                            ...
                            <input type="hidden" name="HdnChangeEmailTmp" id="HdnChangeEmailTmp" value="no"/>
                                     ...
                                     <button type="submit" class="btn btn-default updat_bn" name="submit_doform" id="submit_doform" value="submit" style="default">Update</button>
