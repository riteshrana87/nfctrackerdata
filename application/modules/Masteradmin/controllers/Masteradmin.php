...
public function login()
    {   
        ...
        $GroupNameData = $this->common_model->get_records($table, $fields, '', '', '', '', '', '', '', '', '', $where, '', '');
...
$value_data = base_url();
//$value_data = "https://tracker.newforestcare.co.uk";
$find_appname=array_unique($find_appname);
$find_appname=array_values($find_appname);

if (strpos($value_data, LOCALHOST) !== false) {
        foreach ($find_appname as $group_value) {
    if($group_value == DEVELOPMENT){
        $check_domen_name=array_unique($check_domen_name);
        $check_domen_name = array_values($check_domen_name);
         foreach ($group_array as $key=>$value) {
             $value1=explode('-',$value);
             if($value1[1]==$group_value)
             {
                 $data_main_group=$value;
             }else{
                 if(in_array($value1[1],$check_domen_name)){
                 unset($group_array[$key]);
                 }
             }
         }
        $data_group_data = array_values($group_array);
        foreach ($data_group_data as $app_name) {
            if (strpos($app_name, DEVELOPMENT) == false) {
               $data_group[] = $app_name;
            }
        }
        $match = "rm.status = 1 && rm.is_delete = 0 && rgt.group_name = '".$data_main_group."' &&";
    ...
    }else if (strpos($value_data, DEVTRACKER) !== false) {
        foreach ($find_appname as $group_value) {
    if($group_value == DEVELOPMENT){
       $check_domen_name=array_unique($check_domen_name);
        $check_domen_name = array_values($check_domen_name);
    ...
    if($group_value == PREPRODUCTION){
        ...
        $data_group_data = array_values($group_array);
        foreach ($data_group_data as $app_name) {
            if (strpos($app_name, PREPRODUCTION) == false) {
               $data_group[] = $app_name;
            }
        }
    ...
    if($group_value == PRODUCTION){
        ...
        $data_group_data = array_values($group_array);
        foreach ($data_group_data as $app_name) {
            if (strpos($app_name, PRODUCTION) == false) {
               $data_group[] = $app_name;
            }
        }
...
if (strpos($value_data, LOCALHOST) !== false) {
        foreach ($find_appname as $group_value) {
    if($group_value == DEVELOPMENT){
        foreach ($check_domen_name as $value) {
            $pos = $this->group_name_data($value,$group_array);
            $main_grop_name = @$group_array[$pos];
            if (strpos($main_grop_name, DEVELOPMENT) !== false) {
               $data_main_group = $main_grop_name;
            }else{
                break;
            }
            unset($group_array[$pos]);
        }
    ...
    if($group_value == PREPRODUCTION){
        foreach ($check_domen_name as $value) {
            $pos = $this->group_name_data($value,$group_array);
            $main_grop_name = @$group_array[$pos];
            if (strpos($main_grop_name, PREPRODUCTION) !== false) {
               $data_main_group = $main_grop_name;
            }
            unset($group_array[$pos]);
        }
    ...
    if($group_value == PRODUCTION){
        foreach ($check_domen_name as $value) {
            $pos = $this->group_name_data($value,$group_array);
            $main_grop_name = @$group_array[$pos];
            if (strpos($main_grop_name, PRODUCTION) !== false) {
               $data_main_group = $main_grop_name;
            }
            unset($group_array[$pos]);
        }
  ...
  public function group_name_data($needle, array $haystack)
    {
