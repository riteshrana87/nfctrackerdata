...
<!-- main content start-->
        <div id="page-wrapper">
            <div class="main-page">
                ...
                <div class="row">
                    <form action="<?=base_url('Medical/insert_administer_medication')?>" method="post" id="ppform" name="ppform" data-parsley-validate enctype="multipart/form-data">
                        <?php
                        ...
                        if(!empty($form_data))
                        {
                                        ...
                                        <div class="panel panel-default tile tile-profile">
                                            <div class="panel-body">
                                                <h2><?=!empty($row['label'])?$row['label']:''?> <?=!empty($row['required'])?'<span class="astrick">*</span>':''?>
                                                 ...
                                                 <?php } ?>
                                                 id="<?=!empty($row['name'])?$row['name']:''?>" ><?=!empty($edit_data[0][$row['name']])?nl2br(html_entity_decode($edit_data[0][$row['name']])):(isset($row['value'])?$row['value']:'')?>
                                                 </textarea>
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <h2><?=!empty($row['label'])?$row['label']:''?> <?=!empty($row['required'])?'<span class="astrick">*</span>':''?></h2>
                                                 ...
                                                 <div class="<?=!empty($row['inline'])?'checkbox-inline':'checkbox'?>">
                                                     <label class="<?=!empty($row['toggle'])?'kc-toggle':''?>"><input 
                                                       ...
                                                       name="<?=!empty($row['name'])?$row['name'].'[]':''?>"
                                                       id="<?=!empty($row['name'])?$row['name']:''?>" value="<?=!empty($checkbox['value'])?$checkbox['value']:''?>" <?=(!empty($edit_data[0][$row['name']]))?'checked="checked"':''?> <?=(!empty($checkedValues) && in_array($checkbox['value'], $checkedValues))?'checked="checked"':!empty($checkbox['selected'])?'checked="checked"':''?>  
                                                            <?=!empty($row['required'])?'required=true':''?>
                                                            type="checkbox">
                                    ...
                                    <div class="col-sm-6">
                                            ...
                                            <div class="panel-body">
                                                <h2><?=!empty($row['label'])?$row['label']:''?> <?=!empty($row['required'])?'<span class="astrick">*</span>':''?></h2>
                                                 ...
                                                 foreach ($get_data as $rowdata) {?>
                                                  <option <?=(!empty($edit_data[0][$row['name']]) && $edit_data[0][$row['name']] == $rowdata['login_id'])?'selected':'' ?> value="<?=!empty($rowdata['login_id'])?$rowdata['login_id']:''?>"><?=!empty($rowdata['username'])?$rowdata['username']:''?></option>
                                                <?php
                                        ...
                                        $get_medication_data = $this->common_model->get_medication($ypid);                      
                                                 foreach ($get_medication_data as $rowdata) {?>
                                                  <option <?=(!empty($edit_data[0][$row['name']]) && $edit_data[0][$row['name']] == $rowdata['medication_id'])?'selected':'' ?>  value="<?=!empty($rowdata['medication_id'])?$rowdata['medication_id']:''?>"><?=!empty($rowdata['medication_name'])?$rowdata['medication_name']:''?></option>
                            ...
                            ?>
                             <div class="col-sm-12 capsBtn">
                                <div class="">
                               <input type="hidden" name="administer_medication_id" id="administer_medication_id" value="<?=!empty($edit_data[0]['administer_medication_id'])?$edit_data[0]['administer_medication_id']:''?>">
                                ...
                                <input type="hidden" name="hdn_submit_status" id="hdn_submit_status" value="1"/>
                                    ...
                                    <input type="hidden" name="redirect_flag" id="redirect_flag" value="<?=!empty($redirect_flag)?$redirect_flag:'0'?>"/>
                                    <button type="submit" class="btn btn-default updat_bn" name="submit_medform" id="submit_medform" value="submit" style="default">Update</button>
                                <div class="pull-right for-tab">
                                    ...
                                    <a href="<?=base_url('Medical/index/'.$ypid); ?>" class="btn btn-default"> Back To Meds </a>
                                </div>
                            </div>
                        </div>
                            </div>
