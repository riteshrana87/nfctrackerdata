...
<div class="col-sm-12">
    <div class="panel panel-default tile tile-profile">
        <div class="panel-body">
            ...
            <a href="<?=base_url('PocketMoney/generateExcelFile/'.$ypid); ?>" class="btn btn-default btn-sm">Export to Excel</a>
            <a href="<?=base_url('PocketMoney/DownloadPrint/'.$ypid.'/print'); ?>" target="_blank" class="btn btn-default btn-sm"><i class="fa fa-print"></i> Print </a>
            <?php } ?>
           </div>
                        ...
                        <tbody>
									...
									?>
                                        ...
                                        if(!empty($form_data))
                                        {
                                            ...
                                            foreach ($form_data as $row) {
                                                ...
                                                <?php }elseif (isset($row['subtype']) && $row['subtype'] == 'time') { ?>
                                                <td> 
                                                    <?php 
                                                    if((!empty($data[$row['name']]) && $data[$row['name']] !='0000-00-00')){
                              ...
                              <?php
                                ...
                                <td> <?= (!empty($data[$row['name']]) && $data[$row['name']] !='0000-00-00') ? ((strlen (strip_tags($data[$row['name']])) > 50) ? substr (nl2br(strip_tags($data[$row['name']])), 0, 50)
                                    .'...<a data-href="'.base_url($crnt_view.'/readmore/'.$data['pocket_money_id'].'/'.$row['name']).'" data-refresh="true" data-toggle="ajaxModal" class="btn">read more</a>' : nl2br(html_entity_decode($data[$row['name']]))) :(isset($row['value'])?$row['value']:'') ?></td>
                            ...
                            <?php } } }}?>
                                ...
                                <?php if(checkPermission('PocketMoney','delete')){ ?>
                                    <a onclick="delete_ks('<?php echo $data['pocket_money_id'] ?>','<?php echo $data['yp_id'] ?>');" href="javascript:;"class="btn btn-link btn-blue"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
