...
<div id="page-wrapper">
    <div class="main-page">
        <form action="<?= base_url('PocketMoney/insert') ?>" method="post" id="ksform" name="ksform" data-parsley-validate enctype="multipart/form-data">
            <div class="sticky-heading" id="sticky-heading">
        <h1 class="page-title">
            Pocket Money <small>New Forest Care</small>
            <div class="pull-right">
                <div class="btn-group capsBtn">
                    <button type="submit" onclick="needToConfirm = false;"  class="btn btn-default" name="submit_pmform" id="submit_pmform" value="submit" style="default">Add</button>
                ...
                if (!empty($ks_form_data)) {
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="<?= (!empty($row['subtype']) && $row['subtype'] == 'time') ? 'input-group input-append bootstrap-timepicker' : '' ?><?= (!empty($row['type']) && $row['type'] == 'date') ? (!empty($row['description']) && $row['description'] == 'dob') ? 'input-group input-append date dob' : 'input-group input-append date adddate' : '' ?>">
                                                    <input type="<?= (!empty($row['type']) && $row['type'] == 'number') ? 'number' : ((!empty($row['subtype']) && $row['subtype'] != 'time') ? $row['subtype'] : 'text') ?>"  
                                                           ...
                                                           <?=($row['name'] == 'money_out')?"max =".$total_balance."":''?>
                                    ...
                                    <div class="col-sm-6">
                                        <div class="panel panel-default tile tile-profile">
                                            <div class="panel-body">
                                                <h2><?=!empty($row['label'])?$row['label']:''?> <?=!empty($row['required'])?'<span class="astrick">*</span>':''?></h2>
                                                 ...
                                                 <select class="chosen-select <?=!empty($row['className'])?$row['className']:''?>" data-parsley-errors-container="#errors-container<?= $row['name'] ?>" name="<?=!empty($row['name'])?$row['name']:''?>" id="<?=!empty($row['name'])?$row['name']:''?>" <?=!empty($row['required'])?'required=true':''?>>
                                                 <option value="">Select</option>
                                                 <?php if($row['name'] == 'reason'){
                                                    ?>
                                                    <option value="Earnt from Banding">Earnt from Banding</option>
                                                    <?php 
                                                    } ?>
                                        ...
                                        $get_medication_data = $this->common_model->get_medication($ypid);                      
                                                 ...
                                                 foreach ($row['values'] as $select) {
                                                  ...
                                                  ?>
                                                  <option value="<?=!empty($select['value'])?$select['value']:''?>" <?=(!empty($edit_data[0][$row['name']]) && $edit_data[0][$row['name']] == $select['value'])?'selected="true"':!empty($select['selected'])?'':''?> ><?=!empty($select['label'])?$select['label']:''?></option>
                                                <?php } } } }//select loop ?>
                                        ...
                                        <div class="panel panel-default tile tile-profile">
                                            ...
                                            <input type="file" name="<?=!empty($row['name'])?$row['name'].'[]':''?>" id="<?=!empty($row['name'])?$row['name']:''?>"  class="<?=!empty($row['className'])?$row['className']:''?>" 
                                                ...
                                                $fileViewArray = array(
                                                    ...
                                                    'filePathMain' => $this->config->item('pm_img_base_url') . $ypid,
                                                    'filePathThumb' => $this->config->item('pm_img_base_url_small') . $ypid
                                                );
                    ...
                    ?>
                        ...
                        <div class="col-sm-12 capsBtn">
                                ...
                                <button type="submit" onclick="needToConfirm = false;"  class="btn btn-default updat_bn" name="submit_pmform" id="submit_pmform" value="submit" style="default">Add</button>
                                  <div class="page-title pull-right width-xs-100">
                                  <div class="btn-group ">
