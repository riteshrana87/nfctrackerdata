<script>
    ...
    var ks_id = '<?=!empty($edit_data[0]['rmp_id'])?$edit_data[0]['rmp_id']:''?>';
</script>
<!-- main content start-->
<div id="page-wrapper">
    <div class="main-page">
        <form action="<?= base_url('RiskManagementPlan/insert') ?>" method="post" id="ksform" name="ksform" data-parsley-validate enctype="multipart/form-data">
        ...
        <h1 class="page-title">
            ...
            <div class="pull-right for-tab">
                <div class="btn-group capsBtn">
                    <button type="submit" onclick="needToConfirm = false;"  class="btn btn-default" name="submit_rmpform" id="submit_rmpform" value="submit" style="default"><?=!empty($edit_data[0]['rmp_id'])?'Update':'Create'?></button>
                    <a href="<?=base_url($this->viewname .'/index/'.$ypid)?>" class="btn btn-default" style="default">Back To RMP</a>   
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <?php if ((!empty($row['subtype']) && $row['subtype'] == 'time') || $row['type'] == 'date') {
                                            ...
                                            <div class="row">
                                                <div class="col-sm-4">
                                                <?php } ?>
                                                <div class="<?= (!empty($row['subtype']) && $row['subtype'] == 'time') ? 'input-group input-append bootstrap-timepicker' : '' ?><?= (!empty($row['type']) && $row['type'] == 'date') ? (!empty($row['description']) && $row['description'] == 'dob' && $row['name'] != 'activity_date_from' && $row['name'] != 'activity_date_to') ? 'input-group input-append date dob' : 'input-group input-append date ' : '' ?> <?=($row['name'] == 'activity_date_from')?'add_start_date':'';?> <?=($row['name'] == 'activity_date_to')?'add_end_date':'';?>">
                                                    <input type="<?= (!empty($row['type']) && $row['type'] == 'number') ? 'number' : ((!empty($row['subtype']) && $row['subtype'] != 'time') ? $row['subtype'] : 'text') ?>"  
                                                           class="<?= !empty($row['className']) ? $row['className'] : '' ?> <?= ($row['type'] == 'date'  && $row['name'] != 'activity_date_from' && $row['name'] != 'activity_date_to') ? '' : '' ?> <?= (!empty($row['subtype']) && $row['subtype'] == 'time') ? 'addtime' : '' ?> addtime_data datebox <?=($row['name'] == 'activity_date_from')?'add_start_date':'';?> <?=($row['name'] == 'activity_date_to')?'add_end_date':'';?>"  
                                                           <?= !empty($row['required']) ? 'required=true' : '' ?>
                                                    ...
                                                    <?php if (!empty($row['type']) && $row['type'] == 'date') { ?>
                                                        <span class="input-group-addon boxdate add-on"><i class="fa fa-calendar"></i></span>
                                                    <?php } ?>
                                        ...
                                        <div class="panel panel-default tile tile-profile">
                                            <div class="panel-body">
                                                <h2><?=!empty($row['label'])?$row['label']:''?> <?=!empty($row['required'])?'<span class="astrick">*</span>':''?></h2>
                                                 ...
                                                 <select class="chosen-select <?=!empty($row['className'])?$row['className']:''?>" name="<?=(!empty($row['name']) && (!empty($row['description'])))?$row['name'].'[]':''?>" <?=(!empty($row['description']) && $row['description'] == 'select_multiple_user')?'multiple':''?> id="<?=!empty($row['name'])?$row['name']:''?>" <?=!empty($row['required'])?'required=true':''?>>
                                                 <option value="">Select</option>
                                                 
                                                 <?php if(!empty($row['description']) && ($row['description'] == 'get_user' ||$row['description'] == 'select_multiple_user')){
                                                 $get_data = $this->common_model->get_user(); 
                                                 $userAr = array();
                                                 $userAr = explode(',',$edit_data[0][$row['name']]);
                                                 foreach ($get_data as $rowdata) { ?>
                                                  <option <?=(in_array($rowdata['login_id'], $userAr)?'selected':'')?> value="<?=!empty($rowdata['login_id'])?$rowdata['login_id']:''?>"><?=!empty($rowdata['username'])?$rowdata['username']:''?></option>
                                                <?php
                                    } }else{?>
                                                 <?php if(count($row['values']) > 0) {
                    ...
                    ?>
                        ...
                        <div class="">
                         <input type="hidden" name="draft_data" id="draft_data" value="0">
                                ...
                                <input type="hidden" name="care_home_id" id="care_home_id" value="<?php echo $YP_details[0]['care_home']; ?>">
                                <input type="hidden" name="rmp_id" id="rmp_id" value="<?=!empty($rmp_id)?$rmp_id:''?>">
                                <input type="hidden" name="is_copy" id="is_copy" value="<?=!empty($is_copy)?$is_copy:''?>">
                                <input type="hidden" id="form_secret" name="form_secret"  value="<?php echo createFormToken();?>">

                            <button type="submit" onclick="needToConfirm = false;"  class="btn btn-default updat_bn" name="submit_rmpform" id="submit_rmpform" value="submit" onclick="$('#saveAsDraft').val(0);" style="default"><?=!empty($edit_data[0]['rmp_id'])?'Update':'Create'?></button>
                                
                                <button type="submit" class="btn btn-default updat_bn" name="draft_wrform" id="draft_wrform" style="default" data-value="0" onclick="$('#saveAsDraft').val(1);">SAVE AS DRAFT</button>
