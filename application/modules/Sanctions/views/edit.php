<!-- main content start-->
<?php /* pr($manager_details_of_review['value']);exit; */ ?>
<script>
  ...
  <form action="<?=base_url($this->viewname.'/insert')?>" method="post" id="mdtform" name="mdtform" data-parsley-validate enctype="multipart/form-data">
    <div class="main-page">
      <div class="sticky-heading" id="sticky-heading">
        ...
        <h1 class="page-title">Sanction form</h1>
      </div>
                ...
                <tbody>                     
                  <?php if (isset($information) && count($information) > 0) { ?>
                    <?php foreach ($information as $data) { ?>
                      ...
                      <tr>
                        <td>
                          <input type="checkbox" name="sanction_incident[]" class="incidentCheckbox" value="<?php echo $data['list_main_incident_id']; ?>" <?= (@in_array($data['list_main_incident_id'], $sanction_incident))? 'checked':'' ?>>
                        </td>
                        <td> 
      ...
      <div class="row">
                    ...
                    <select data-parsley-errors-container="#errors-<?= $row['name'] ?>" class='form-control chosen-select' id='<?= $row['name'] ?>' name='<?= $row['name'] ?>'>
                      ...
                      <?php if(!empty($bambooNfcUsers)) {
                        foreach ($bambooNfcUsers as $select) {  ?>
                          <option value="<?php echo $select['user_type'].'_'.$select['user_id'];?>" <?php if(isset($edit_data[0][$row['name']]) && ($edit_data[0][$row['name']] == $select['user_type'].'_'.$select['user_id'])){echo 'selected="true"';}?>> <?php echo $select['first_name'] . ' ' . $select['last_name'] . ' - ' . $select['email'] ;?> </option>
                        <?php } } ?>
                      </select>
                    ...
                    <div class="panel-body">
                      ...
                      <select <?=(!empty($row['className']) && $row['className'] == 'bamboo_lookup_multiple')?'multiple':''?> data-parsley-errors-container="#errors-<?= $row['name'] ?>" class='form-control chosen-select' id='<?= $row['name'] ?>' name='<?=(!empty($row['name']) && (!empty($row['className'])))?$row['name'].'[]':''?>'>
                        ...
                        <?php if(!empty($bambooNfcUsers)) {
                          $userAr = array();
                          foreach ($bambooNfcUsers as $select) {  ?>
                            <option value="<?php echo $select['user_type'].'_'.$select['user_id'];?>" <?=(in_array($select['user_type'].'_'.$select['user_id'], $edit_data[0][$row['name']])?'selected':'')?>> <?php echo $select['first_name'] . ' ' . $select['last_name'] . ' - ' . $select['email'] ;?> 
                          </option>
                        <?php } } ?>
                      </select>
                      <span id="errors-<?= $row['name'] ?>"></span>
                       ...
                       <select multiple <?=(!empty($row['description']) && $row['description'] == 'bamboo_lookup_multiple')?'multiple':''?> data-parsley-errors-container="#errors-<?= $row['name'] ?>" class='form-control chosen-select' id='<?= $row['name'] ?>' name='<?=(!empty($row['name']) && (!empty($row['description'])))?$row['name'].'[]':''?>'>
                        ...
                        <?php if(count($row['values']) > 0) {
                          ...
                          foreach ($row['values'] as $select) {  ?>
                            <option id="<?=!empty($select['value'])?$select['value']:''?>" data-detail="<?= !empty($select['label'])?$select['label']:'' ?>" value="<?=!empty($select['value'])?$select['value']:''?>" <?=(in_array($select['value'],$edit_data[0][$row['name']])?'selected':'')?>><?=!empty($select['label'])?$select['label']:''?> 
                          </option>
                        <?php } } ?>
                      </select>
                    <?php }else{ ?>
                       ...
                       <?php if(count($row['values']) > 0) {
                         foreach ($row['values'] as $select) {
                           if(!empty($select['label'])) {
                            ?>
                            <option id="<?=!empty($select['value'])?$select['value']:''?>" data-detail="<?= !empty($select['label'])?$select['label']:'' ?>" value="<?=!empty($select['value'])?$select['value']:''?>" <?php if(isset($row['value']) && ($edit_data[0][$row['name']] == $select['value'])){echo 'selected="true"';}?> ><?=!empty($select['label'])?$select['label']:''?></option>
                          <?php } } } //select loop ?>
                        </select>
                      <?php } ?> 
        ...
        <?php } else if($row['type'] == 'file'){ ?>
          ...
          <?php
                  ...
                  <div class="col-sm-12">
                    <div class="capsBtn">
                      <input type="hidden" name="sanctions_id" id="sanctions_id" value="<?=!empty($sanctions_id)?$sanctions_id:''?>">
                      ...
                      <button type="submit" onclick="needToConfirm = false;" class="btn btn-default updat_bn" name="submit_snform" id="submit_snform" value="submit" style="default"><?=!empty($edit_data[0]['yp_id'])?'UPDATE':'ADD'?></button>
