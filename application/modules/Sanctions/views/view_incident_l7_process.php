<div class="panel panel-default">
    <div class="panel-heading" role="tab" id="headingSeven">
        <h4 class="panel-title">
            <a class="collapsed accordion-toggle text-uppercase" role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseSeven" aria-expanded="false" aria-controls="collapseSeven">
                            ...
                            <div class="row">
                                <div class="col-sm-4">
                                   <?= (isset($l7_report_compiler))? $l7_report_compiler:$loggedInUser['FIRSTNAME'].' '.$loggedInUser['LASTNAME'] ?>                               
                                    <input type="hidden" name="l7_report_compiler" class="form-control" value="<?= (isset($l7_report_compiler))? $l7_report_compiler:$loggedInUser['FIRSTNAME'].' '.$loggedInUser['LASTNAME'] ?>">
                                </div>                                    
                            </div>
                        </div>
                    </div>
                    ...
                    foreach ($l7_form_data as $row) {
                        ...
                        if ($row['type'] == 'textarea') { ?>
                            ...
                            <div class="col-sm-12">
                                    <div class="panel panel-default tile tile-profile">
                                        <div class="panel-body">
                                            <h2>Safeguarding updates</h2>
                                        </div>
                                    </div>
                                </div>
                                <div class="panel-body" id="add_safeguarding">
                                    <?php if(!empty($l7sequence_data)){ 
                                        $i=1;
                                        foreach ($l7sequence_data as $key=>$value) { ?>
                                        <div class="row" id="item_new_safeguarding_<?php echo $i; ?>">
    <div class="col-sm-12">
    <div>
        <div class="panel-body">
            <div class="col-sm-3"><label>Sequence Number</label></div>
            <div class="col-sm-9">
                 <?php if (!empty($preveditl7Data)) {
    ...
    <div>
        <div class="panel-body">
            <div class="col-sm-3"><label>Update By</label></div>
            <div class="col-sm-9">
    ...
    <div class="col-sm-12">
        <div>
            <div class="panel-body">
                <div class="col-sm-3"><label>Daily action taken</label></div>
                        <div class="col-sm-9">
                              <?php if (!empty($preveditl7Data)) {
    ...
    <div class="col-sm-12">
        <div>
            <div class="panel-body">
                <div class="col-sm-3"><label>Daily action outcome</label></div>
                <div class="col-sm-9">
                <?php if (!empty($preveditl7Data)) {
    ...
    <div class="col-sm-12">
    <div>
        <div class="panel-body"><div class="col-sm-3"><label>Supporting documents</label></div>
            <div class="col-sm-9">
                <h2></h2>
                                                <?php
                                                $fileViewArray_doc = array(
                                                    'fileArray' => (isset($value['l7supporting_documents'.$i]) && !empty($value['l7supporting_documents'.$i]))? $value['l7supporting_documents'.$i] : '',
                                                    'filePathMain' => $this->config->item('aai_img_base_url') . $ypId,
                                                    'filePathThumb' => $this->config->item('aai_img_base_url_small') . $ypId,
                                                    'deleteFileHidden' => 'hidden_'.'l7supporting_documents'.$i
                                                );
                                                echo getFileViewPrint($fileViewArray_doc); ?>
                                            <input type="hidden" name="<?=!empty('l7supporting_documents'.$i)?'hidden_'.'l7supporting_documents'.$i:''?>" id="<?=!empty('l7supporting_documents'.$i)?'hidden_'.'l7supporting_documents'.$i:''?>" value="">
            </div>
        </div>
    </div>
    </div>
    <div class="col-sm-12">
        <div>
            <div class="panel-body"><div class="col-sm-3"><label>Date</label></div>
                <div class="col-sm-9">
                    <?php if (!empty($preveditl7Data)) {
                                        $diff = new HtmlDiff($l7seqresult_prev[$key]['l7date_safeguarding'],$value['l7date_safeguarding']);
                                        $diff->build();
                                        echo $diff->getDifference();
                                        ?>
                                     <?php } else { ?>
    ...
    <hr>
            ...
            ?>     
                         ...
                         <?php if($row['description'] == 'bamboo_lookup'){ ?>
                                    ...
                                    <?php if (!empty($preveditl7Data) && $preveditl7Data["$dd"]['value'] != $row['value']) {
                                        $diff = new HtmlDiff('',$select['first_name'] . ' ' . $select['last_name'] . ' - ' . $select['email']);
                                        $diff->build();
                                        echo $diff->getDifference();
                                    ?>
                                            ...
                                            <div class="panel panel-default tile tile-profile">
                                                <div class="panel-body">
                                                    <h2><?=!empty($row['label'])?$row['label']:''?> <?=!empty($row['required'])?'<span class="astrick">*</span>':''?></h2>
                                                        <?php if(!empty($bambooNfcUsers)){
                                                            foreach ($bambooNfcUsers as $select){ 
                                                        if(!empty($preveditl7Data)){
                                                        $staff_present = array_diff($row['value'],$preveditl7Data["$dd"]['value']);
