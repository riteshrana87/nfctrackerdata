...
// user signoff script
function manager_request(YPId,CPT_ID){
    ...
    BootstrapDialog.show(
        {
            ...
            buttons: [{
                ...
                action: function(dialog) {
                    $("input[name='cpt_signoff']").prop("checked", false);
                    dialog.close();
                }
            }, {
                label: 'ok',
                action: function(dialog) {
                    window.location.href = baseurl +'/CarePlanTarget/manager_review/' + YPId + '/' + CPT_ID;
                    dialog.close();
                }
            }]
        });
...
function signoff_request(YPId,CPT_ID){
    var delete_meg ="Please select OK to authorise this document.";
    BootstrapDialog.show(
        {
            title: 'Information',
            ...
            buttons: [{
                ...
                action: function(dialog) {
                    ...
                    window.location.href = baseurl +'/CarePlanTarget/signoff_review_data/' + YPId + '/' + CPT_ID + '/' + EMAIL;
                    dialog.close();
                }
            }]
        });
    ...
    BootstrapDialog.show(
        {
            ...
            buttons: [{
                    ...
                    label: 'Ok',
                    action: function (dialog) {
                        window.location.href = baseurl + "/CarePlanTarget/deletedata/" + ks_id + '/' + yp_id;
                        dialog.close();
                    }
...
function conclude_ks(cpt_id,yp_id)
{
    BootstrapDialog.show(
        {
            title: 'Confirm!',
            message: "<strong> Are you sure want to Conclude Care Plan Target ? <strong>",
            buttons: [{
                    label: 'Cancel',
                    action: function (dialog) {
                        dialog.close();
                    ...
                    action: function (dialog) {                        
                        ...
                        $.ajax({
                            type: "POST",
                            url: baseurl +'/CarePlanTarget/concludeCPT',
                            data: {
                                cpt_id: cpt_id,yp_id: yp_id
                            },
                            success: function (html) {
                               $("#conclude_option_"+cpt_id).hide();
                               $("#concluded_option_"+cpt_id).show();
                            },
...
$('body').on('change', '#user_type', function (e) {
    ...
    $.ajax({
        type: "POST",
        url: baseurl +'/CarePlanTarget/getUserTypeDetail',
        data: {
    ...
    BootstrapDialog.show(
        {
            ...
            buttons: [{
                ...
                label: 'ok',
                action: function(dialog) {
                    window.location.href = baseurl + 'CarePlanTarget/resend_external_approval/'+ id+'/'+ypid+'/'+ksid;
                    dialog.close();
                }
            }]
        });
